...
function installSignalHandlers(shutdownFn, resyncFn, reloadFn) {
  ...
  // v0.8.19 bisection: maximally minimal SIGTERM handler — sync process.exit(0)
  // with no shutdown work, no async chain. v0.8.18 traces proved process.exit(0)
  // IS reached but container still exits 143. If this minimal handler also exits
  // 143, the SEA Node runtime is silently ignoring exit code 0 from a SIGTERM
  // handler context. If it exits 0, the shutdown chain is the cause and we
  // need to fire-and-forget shutdown rather than await it.
  process.on('SIGTERM', () => {
    process.stderr.write('[shutdown-trace] SIGTERM minimal handler — process.exit(0)\n');
    process.exit(0);
  });
  process.on('SIGINT', () => {
    process.stderr.write('[shutdown-trace] SIGINT minimal handler — process.exit(0)\n');
    process.exit(0);
  });
  // Suppress unused-vars warning while bisecting — orchestrator + shutdownFn
  // wired back in once the SEA exit-code mystery is resolved.
  void orchestrator; void shutdownFn;
