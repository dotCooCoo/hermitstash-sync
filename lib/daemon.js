...
function installSignalHandlers(shutdownFn, resyncFn, reloadFn) {
  ...
  async function handleTerm(sig) {
    ...
    _shuttingDown = true;
    process.stderr.write('[shutdown-trace] ENTER ' + sig + '\n');
    log.info(`Received ${sig} — graceful shutdown`);
    try {
      await orchestrator.shutdown();
    } catch (err) {
      process.stderr.write('[shutdown-trace] SHUTDOWN_THREW: ' + (err && err.message) + '\n');
      process.exit(1);
      return;
    }
    process.stderr.write('[shutdown-trace] PRE_EXIT_0 — calling process.exit(0)\n');
    process.exit(0);
    process.stderr.write('[shutdown-trace] POST_EXIT_0 — should be unreachable\n');
  }
