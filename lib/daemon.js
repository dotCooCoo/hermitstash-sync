...
// codebase-patterns:allow-file sea-sigterm-bug — handleTerm composes b.appShutdown phase orchestration with explicit process.exit after orchestrator.shutdown resolves. SEA Node bug bisected v0.8.15 → v0.8.20: process.exit(0) inside a SIGTERM handler in a SEA-packaged binary still exits 143 (signal-default) instead of 0. Dockerfile entrypoint remaps via `tini -e 143` so container deployments see exit 0. Foreground / systemd / non-tini supervisors still see 143; tracked for upstream-Node investigation in v0.8.21+.
