  ...
  const orchestrator = b.appShutdown.create({
    ...
    phases: [
      ...
      },
      {
        ...
        // worker, sd_notify socket) keep the loop alive past the SIGTERM
        // handler return — Node then exits 143 (128 + SIGTERM) rather
        // than the 0 the shutdown earned. v0.8.15 tried setImmediate(()
        // => process.exit(0)) but the immediate callback never landed in
        // time. Calling process.exit synchronously inside the phase
        // terminates before Node can take the signal-default path.
        // Acceptable here because the engine + pidfile phases ahead of
        // this one have already done all the real teardown — flushing
        // logs, closing sockets, removing the pidfile — so a hard exit
        // at this point loses nothing operator-visible.
        name: 'forceExit',
        run:  () => {
          process.exit(0);
        },
