...
async function daemonize() {
  ...
  // b.daemon.start reports success unconditionally — it has no post-spawn
  // liveness probe, so a child that dies during boot (bad config, argv bug,
  // port clash) would leave the operator with a false "Daemon started" and,
  // on Windows (stdio 'ignore'), no trace at all. Poll the PID briefly so an
  // instantly-dying child fails loudly. Best-effort: a child that dies later
  // (e.g. keychain errors after this window) still needs `status`/the log.
  let alive = true;
...
function writePid() {
  ...
  // here. The vendored pidLock exempts a pidfile recording process.pid from
  // the held-error, but it still unlinks and O_EXCL-recreates the file rather
  // than adopting the existing inode — so for the parent-handoff case there is
  // a momentary no-pidfile window. A concurrent `stop` racing that narrow
  // window can observe no live owner and report 'No daemon running'; the
  // impact is benign and self-healing (re-run `stop`). The isRunning() guard
  // in cmdStart only prevents a second `start` from launching — it does NOT
  // serialise against `stop`. Eliminating the window entirely is an upstream
  // pidLock change (adopt the existing inode when it already records
  // process.pid), tracked against blamejs rather than patched in the vendor
  // tree.
