...
class HttpClient {
  constructor(config, apiKey) {
    ...
    // instead of each getting the full rate. `0` = unlimited (the
    // limiter returns a pass-through Transform). Operators set
    // uploadBytesPerSec / downloadBytesPerSec in config.json.
    this._uploadLimiter   = throttle.createLimiter({ bytesPerSec: config.uploadBytesPerSec | 0 });
    this._downloadLimiter = throttle.createLimiter({ bytesPerSec: config.downloadBytesPerSec | 0 });
  }

  _rebuildAgent() {
    // allow:silent-catch — best-effort destroy on rebuild; outgoing requests carry their own AbortSignal and a destroyed agent is recreated on this very line
  ...
  async uploadFile(bundleId, relativePath, filePath) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const throttled = fileStream.pipe(throttle.createTransform(this._uploadLimiter));
  ...
  async downloadFile(fileId, destPath, expectedChecksum) {
    ...
    const tmpPath = destPath + '.tmp.' + b.crypto.generateToken(4);
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = mod.request(reqOpts, res => {
        ...
        const throttled = res.pipe(throttle.createTransform(this._downloadLimiter));
        throttled.pipe(ws);
        ...
        ws.on('finish', () => {
          try {
            ...
            nodeFs.renameSync(tmpPath, destPath);
            resolve(destPath);
          } catch (err) {
            reject(err);
          }
