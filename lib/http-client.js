 ...
 * @returns {object} tls options ready to spread into nodeHttps.request / tls.connect
 */
...
class HttpClient {
  ...
  _rebuildAgent() {
    ...
    if (this._isHttps) {
      ...
      // cert/key/ca pass through to the underlying nodeHttps.Agent options.
   ...
   * binary download) keep the raw `nodeHttps.request` path because
