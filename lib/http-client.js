 ...
 * @param {?object} extras — additional tls options merged LAST. Never feed
 *   operator- or config-supplied data in here: merged last it overrides
 *   everything set above, and nearly every tls.connect option can defeat server
 *   identity — `ca` replaces the trust anchors, `servername` picks the name the
 *   identity check validates against, `minVersion`/`ciphers` allow a TLS 1.2
 *   downgrade — not just the obvious `rejectUnauthorized`. Callers pass
 *   compile-time constants only; the WebSocket transport ignores config-supplied
 *   TLS options for exactly this reason (see _configTlsOverrides in ws-client.js).
...
class HttpClient {
  ...
  _rebuildAgent() {
    ...
    if (this._isHttps) {
      ...
      // Only `ecdhCurve` is set, and it is set above through create(). Node has
      // no `groups` TLS option — a bogus value assigned to it is accepted and
      // discarded, where a bogus `ecdhCurve` throws — so a second assignment
      // here would look like a redundant handle on the preference while
      // changing nothing, and narrowing it later would silently do nothing.
      // Same reason `curves` and `namedCurves` are never set. See the note at
      // _tlsOptions() for the same rule on the per-request path.
      // Optional SPKI pinning — when config.pinnedServerSpki is set, every TLS
      // handshake must produce a leaf cert whose SPKI hash matches one of the
      // configured pins. Layered ON TOP of the default chain + hostname checks;
      // we don't bypass them — EXCEPT in tunnel mode, where the leaf's SAN match
      // is skipped (the tunnel + pin + mTLS bind identity) while the pin + chain
      // stay enforced.
