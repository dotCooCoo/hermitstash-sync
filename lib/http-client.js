...
const { ml_kem768 } = require(path.join(__dirname, '..', 'vendor', 'noble-pq.cjs'));
const { xchacha20poly1305 } = require(path.join(__dirname, '..', 'vendor', 'noble-ciphers.cjs'));
...
class HttpClient {
  constructor(config, apiKey) {
    ...
    // ML-KEM-768 keypair for hybrid ECIES key exchange
    this._kemKeypair = ml_kem768.keygen();
    ...
    if (this._isHttps) {
      ...
      if (config.mtls && config.mtls.key) {
   ...
   */
  async initSession() {
    const resp = await this._rawRequest('POST', '/drop/init', {
      uploaderName: 'session-init',
      fileCount: 0,
      bundleType: 'sync',
    }, true); // true = include KEM header
   ...
   * ML-KEM-768 decapsulation + ECDH P-384 + HKDF-SHA3-256 + XChaCha20.
   */
  _hybridDecrypt(ekBase64url, epkBase64url, kemBase64url) {
    // ML-KEM-768 leg: decapsulate
    var kemCt = Buffer.from(kemBase64url, 'base64url');
    var ssKem = ml_kem768.decapsulate(new Uint8Array(kemCt), this._kemKeypair.secretKey);
    ...
    var wrappingKey = crypto.hkdfSync('sha3-256', combined, '', HYBRID_HKDF_INFO, 32);

    // Decrypt session key with XChaCha20-Poly1305
  ...
  _rawRequest(method, urlPath, body, includeKemHeader, extraHeaders) {
    return new Promise((resolve, reject) => {
      ...
      var req = mod.request(reqOpts, res => {
        this._extractCookie(res.headers);
        var respBody = '';
        res.on('data', chunk => { if (respBody.length < 65536) respBody += chunk; });
