...
// Enforces the post-quantum floor on a freshly handshaked socket.
//
// Node reports a classical group as a named ECDH key and an ML-KEM hybrid as
// an empty object — it does not model the hybrid as ECDH at all. So a name
// that does not carry "MLKEM" means the peer offered no hybrid and the
// handshake fell back: a downgrade.
//
// The negotiated group is always logged. On the enforced path a classical
// group destroys the socket, naming the group and host, so a downgraded or
// intercepted connection cannot proceed — unless
// HERMITSTASH_ALLOW_CLASSICAL_TLS turns that into the same warning the
// observe-only path emits. The self-update downloader is observe-only: its CDN
// legitimately speaks no ML-KEM, and failing there would break self-update.
//
// Returns true only when it tore the socket down, so the WebSocket channel can
// branch on that rather than reading a private field to detect the failure.
function assertNegotiatedGroupPqc(socket, host, opts) {
  ...
  // A post-quantum group must now identify itself POSITIVELY. Node 24.19+
  // reports an ML-KEM hybrid as { type: 'TLSGroup', name: '<IANA group>' }
  // (#64119); the MLKEM name check stays as a belt-and-braces second signal.
  if (info.type === 'TLSGroup' || (typeof group === 'string' && /MLKEM/i.test(group))) {
    return false;                                     // at the floor
  }

  if (!group) {
    // Nothing reported, which is not the same as "hybrid, therefore fine".
    // On Node before 24.19 an ML-KEM hybrid reported no name, so an empty
    // object really was ambiguous and the floor had to fail open. From 24.19 a
    // hybrid self-identifies, and per the tls docs an empty key-info means only
    // that the key exchange was NOT EPHEMERAL. That is not
    // a post-quantum exchange and it carries no forward secrecy, so it is
    // refused under enforcement like any other non-PQ negotiation.
    log.warn(
      'TLS reported no ephemeral key exchange against ' + host +
      ' — the connection is neither post-quantum nor forward-secret',
      { host: host }
    );
    if (enforce && !ALLOW_CLASSICAL_TLS) {
      socket.destroy(new Error(
        'server negotiated a non-ephemeral TLS key exchange for ' + host +
        ...
        'non-post-quantum connection deliberately.'
      ));
      return true;
    }
    return false;
...
/**
 * TLS options for every https and tls consumer here. The post-quantum groups
 * and the TLS 1.3 floor are always set and callers cannot opt out — that is
 * the point of it being in one place. mTLS material may be a path or a
 * pre-loaded Buffer; the WebSocket client passes buffers so a reconnect does
 * not re-read them.
 *
 * `extras` is merged LAST, so never route operator- or config-supplied data
 * through it. Nearly every option there can defeat server identity, not just
 * the obvious one: `ca` replaces the trust anchors, `servername` picks the name
 * the identity check runs against, and `minVersion` or `ciphers` permit a
 * downgrade. Callers pass compile-time constants only, which is why the
 * WebSocket transport ignores config-supplied TLS options outright.
 */
// Setting `ca` REPLACES Node's default trust store rather than adding to it, so
// passing the enrollment CA alone silently breaks validation for every server
// certificate not anchored by it. The system roots, NODE_EXTRA_CA_CERTS and the
// enrollment CA are therefore combined, so an existing trust chain keeps
// working and a self-hosted server signed by the enrollment CA validates with
...
function buildTlsOptions(mtls, extras) {
  // `ecdhCurve` only. Node has no `groups` TLS option — configSecureContext
  // never reads one, and an unknown key is accepted without error, so passing
  // `groups` alongside looked like belt-and-braces while contributing nothing.
  // `ecdhCurve` is validated (a bad value throws), which is what makes the
  // group list load-bearing rather than advisory.
  const opts = {
...
// A certificate's SubjectPublicKeyInfo hash, in the `sha256/<base64>` form most
// pin tooling uses. The pin binds to the public key rather than the certificate,
// so a rotation that reuses the keypair keeps it valid — that key continuity is
// the property being relied on.
//
// Derived in one place because the verification below and the diagnostics
// bundle both depend on it, and the operator copies one side's output into the
// other's config: a drift in digest, export type or encoding between two copies
// would mint pins that never match.
...
// Build a `checkServerIdentity` override that enforces SPKI pinning on top of
// Node's default chain + hostname checks. With `opts.relaxHostname` (tunnel
// mode) the leaf's hostname/SAN match is skipped but the pin is still enforced.
// Returns `undefined` (default trust path) only when there are no pins AND the
// hostname check is not being relaxed.
//
// Node runs this AFTER chain validation (rejectUnauthorized), so the CA chain is
// always verified regardless — this override governs only the identity binding
// (hostname/SAN + SPKI pin), never the chain.
function _buildCheckServerIdentity(pinList, opts) {
  const relaxHostname = !!(opts && opts.relaxHostname);
  const havePins = Array.isArray(pinList) && pinList.length > 0;
  if (!havePins && !relaxHostname) return undefined;
  const pins = havePins ? pinList.slice() : [];
  return function (hostname, cert) {
    if (relaxHostname) {
      // Tunnel mode: the connection rides inside a Tailscale/WireGuard tunnel, so
      // the server's identity is bound by the pinned SPKI + the mTLS chain + the
      // PQC group floor — NOT the leaf's SAN (HS's external leaf won't carry the
      // tunnel's MagicDNS name). Skip the hostname/SAN match. NEVER skip it with
      // no pin: that would accept any cert chaining to a trusted CA with no
      // identity binding. config.validate refuses tunnelMode without a pin; this
      // is the fail-closed twin at the TLS layer.
      if (pins.length === 0) {
        return new Error(
          'tunnel mode: refusing to skip the TLS hostname check with no pinned ' +
          'server SPKI (pinnedServerSpki is empty)');
      }
    } else {
...
/**
 * TLS-probe the server's leaf certificate and return its SPKI pin
 * ("sha256/<base64>"), for trust-on-first-use capture at enrollment (tunnel
 * mode). Connects with the PQC group preference + TLS 1.3 pin and
 * rejectUnauthorized:false — the same first-contact posture as
 * buildEnrollmentAgent, because the enrollment channel is itself the trust-
 * establishment step. Resolves the pin string; rejects on a connect failure, a
 * timeout, or a plaintext (http:) server (a pin is meaningless without TLS).
 */
function probeServerLeafSpki(serverUrl, opts) {
  const timeoutMs = (opts && opts.timeoutMs) || C.TIME.seconds(15);
  let parsed;
  try {
    parsed = b.safeUrl.parse(serverUrl, { allowedProtocols: b.safeUrl.ALLOW_HTTP_ALL });
  } catch (e) {
    return Promise.reject(new Error('cannot parse the server URL to capture an SPKI pin: ' + (e && e.message)));
  }
  if (parsed.protocol !== 'https:') {
    return Promise.reject(new Error(
      'cannot capture a server SPKI pin over ' + parsed.protocol + ' — tunnel mode reaches the ' +
      'HermitStash server over https:// (TLS terminated at the server, inside the tunnel), ' +
      'not a plaintext or serve-terminated port'));
  }
  const host = parsed.hostname;
  const port = parsed.port ? parseInt(parsed.port, 10) : 443;
  // Node refuses SNI for an IP literal (RFC 6066 §3); leave servername unset there.
  const servername = nodeNet.isIP(host) ? undefined : host;
  return new Promise((resolve, reject) => {
    ...
    const done = (err, pin) => {
      if (settled) return;
      settled = true;
      try { if (socket) socket.destroy(); } catch (_e) { /* allow:silent-catch — best-effort probe socket teardown */ }
      if (err) reject(err); else resolve(pin);
    };
    socket = nodeTls.connect({
      host,
      port,
      servername,
      ecdhCurve: TLS_GROUPS,
      minVersion: TLS_MIN_VERSION,
      rejectUnauthorized: false,  // allow:tls-verify-disabled — first-contact TOFU SPKI capture at enrollment; the captured pin governs every subsequent connect
    }, () => {
      try {
        const cert = socket.getPeerCertificate(false);
        if (!cert || !cert.raw) return done(new Error('the server presented no TLS leaf certificate to pin'));
        const pin = spkiPin(cert);
        if (!pin) return done(new Error('could not derive an SPKI pin from the server certificate'));
        done(null, pin);
      } catch (e) { done(e); }
    });
    socket.setTimeout(timeoutMs, () => done(new Error('timed out connecting to ' + host + ':' + port + ' to capture the server SPKI pin')));
    socket.on('error', (e) => done(e));
  });
}

/**
 * Collects a response body as text, capped so a hostile server cannot grow it
 * without bound, destroying the stream the moment a chunk would overflow. For
 * JSON and text endpoints only — a binary download must stream.
 *
 * The cap is a true UTF-8 byte ceiling. Counting `body.length` instead counts
 * UTF-16 code units, which inflates the real ceiling several-fold on multibyte
 * content, and overshoots by a chunk because the check runs before the append.
 */
...
// which rides out those transient Windows / Dropbox lock codes on a bounded
// backoff, so this module does not carry its own. RENAME_LOCK_CODES (above) is
...
class HttpClient {
  constructor(config, apiKey) {
    ...
    // Built on the first request that needs it, caching the server's published
    // public key and a per-session counter for replay defence.
    //
    // The promise memoizes an in-flight bootstrap, so concurrent first callers
    // share one key fetch instead of racing two sessions. The chain serializes
    // requests against that one client, keeping its strictly monotonic counters
    // in lockstep — out-of-order responses would trip the replay check.
    ...
    // One bucket per direction, so concurrent transfers share the configured
    // budget rather than each taking the full rate. Zero means unlimited.
    //
    // Not bitwise `| 0`: ToInt32 wraps any value at or above 2^31 to a negative
    // number, so a large but perfectly valid cap — a multi-gigabit LAN limit —
    // would wrap and silently disable the throttle altogether.
  ...
  _rebuildAgent() {
    ...
    if (this._isHttps) {
      ...
      // `ecdhCurve` is the only group option set, and it is set once above.
      // Node has no `groups` option — a bogus value there is accepted and
      // discarded, where a bogus `ecdhCurve` throws — so assigning it here
      // would look like a handle on the preference while changing nothing, and
      // narrowing it later would silently do nothing. Same for `curves` and
      // `namedCurves`.
      //
      // Pinning layers on top of the default chain and hostname checks rather
      // than replacing them. Tunnel mode is the one exception: the leaf's name
      // match is skipped, because the tunnel, the pin and mTLS bind identity
      // between them, while the pin and the chain stay enforced.
      const pin = _buildCheckServerIdentity(this._config.pinnedServerSpki, { relaxHostname: !!this._config.tunnelMode });
  ...
  async uploadFile(bundleId, relativePath, filePath) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Without this teardown a request error or timeout leaves the read
      // stream's descriptor open, which accumulates toward EMFILE on a
      // long-running daemon and keeps the source file locked on Windows.
      //
      // The flag matters because teardown is re-entrant — several handlers fire
      // on one failure — and the descriptor must be closed exactly once. A
      // second close targets a raw number the event loop may already have
      // reused for another transfer or a socket, silently closing something
      // unrelated.
      let srcClosed = false;
      const cleanupSrc = () => {
...
module.exports.probeServerLeafSpki = probeServerLeafSpki;
