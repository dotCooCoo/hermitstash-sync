...
function assertNegotiatedGroupPqc(socket, host, opts) {
  ...
  if (!group) {
    // Nothing reported. This used to be read as "hybrid, therefore fine",
    // because before 24.19 an ML-KEM hybrid reported no name and the empty
    // object was genuinely ambiguous — so the floor had to fail OPEN here. It
    // no longer is: a hybrid self-identifies, and per the tls docs an empty
    // key-info means only that the key exchange was NOT EPHEMERAL. That is not
