...
const bSafeUrl   = require('../vendor/blamejs/lib/safe-url');
const bPqcAgent  = require('../vendor/blamejs/lib/pqc-agent');
const bHttpClient = require('../vendor/blamejs/lib/http-client');
// Index.js attaches `encrypted` onto httpClient lazily; pull the helper
// directly from api-encrypt so we don't need to load the full b namespace.
...
// Parse blamejs's FrameworkError("HTTP_ERROR") message back into
// { code, body }. The wire format is "HTTP <N>" or "HTTP <N>: <body slice>".
// String-based parsing instead of a regex with [\s\S]* tail — the regex form
// is linear-time at runtime (anchored end), but ESLint's detect-unsafe-regex
// is conservative around any unbounded class match, so we sidestep it.
function _parseHttpStatusFromMessage(msg) {
  if (!msg || typeof msg !== 'string' || !msg.startsWith('HTTP ')) return null;
  const colonIdx = msg.indexOf(': ', 5);
  const statusEnd = colonIdx !== -1 ? colonIdx : msg.length;
  const code = parseInt(msg.substring(5, statusEnd).trim(), 10);
  if (!Number.isFinite(code)) return null;
  return {
    code: code,
    body: colonIdx !== -1 ? msg.substring(colonIdx + 2) : '',
  };
}
...
class HttpClient {
  constructor(config, apiKey) {
    ...
    this._serverUrl = bSafeUrl.parse(config.server, {
  ...
  async _getEncClient() {
    ...
    this._encClient = bHttpClientEncrypted({
  ...
  async _rawRequest(method, urlPath, body, extraHeaders) {
    ...
    try {
      ...
      if (err && err.code === 'HTTP_ERROR') {
        ...
        // Preserves the legacy contract that 4xx/5xx come back as a normal
        // { statusCode, body } so test code can assert on resp.statusCode.
        const parsed = _parseHttpStatusFromMessage(err.message);
        if (parsed) {
          return { statusCode: parsed.code, headers: {}, body: parsed.body };
        }
      }
      throw err;
    }
...
module.exports.parseHttpStatusFromMessage = _parseHttpStatusFromMessage;
