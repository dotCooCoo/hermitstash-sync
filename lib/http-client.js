...
// /drop/init, /drop/finalize). The encrypted wrapper forwards idleTimeoutMs
// to the base request (a zero-progress cap) but drops timeoutMs, so the
// wall-clock bound is enforced client-side via Promise.race. These are
// low-volume metadata ops, so a tight wall-clock ceiling is fine and keeps a
// trickling peer from wedging the serialized _encChain queue.
...
class HttpClient {
   ...
   * and retry once.
   */
  // Wall-clock cap for an encrypted request. The blamejs encrypted wrapper
  // forwards only idleTimeoutMs (a zero-progress cap) to the base request and
  // exposes no abort handle, so a peer that keeps a connection trickling
  // forward progress under the idle window escapes the intended bound. Bound
  // the request promise with b.safeAsync.withTimeout, which races against a
  // deadline and clears its timer on settle (no leaked timer), so the caller —
  // and the serialized _encChain queue behind it — is released at the ceiling.
  // The underlying request keeps running in the background on its own idle cap
  // (the wrapper can't be cancelled), exactly the withTimeout contract. Its
  // async/timeout rejection is translated to the actionable, transient
  // ENC_WALL_CLOCK_TIMEOUT error the caller surfaces and a retry can clear.
  _withWallClock(promise, timeoutMs, label) {
    return b.safeAsync.withTimeout(promise, timeoutMs, { name: label }).catch((e) => {
      if (e && e.code === 'async/timeout') {
        const err = new Error(
          'Encrypted request ' + label + ' exceeded the ' + timeoutMs +
          'ms wall-clock limit — the server accepted the connection but did ' +
          'not return a complete response in time. Retry; if it persists the ' +
          'server may be overloaded or a network path is dropping bytes.'
        );
        err.code = 'ENC_WALL_CLOCK_TIMEOUT';
        err.permanent = false;   // transient: a retry can succeed
        throw err;
      }
      throw e;
    });
  ...
  async _encryptedRequestOnce(method, urlPath, bodyData, retried) {
    ...
    let resp;
    try {
      // b.httpClient.encrypted only forwards a fixed key allowlist to the base
      // request (allowedProtocols / allowInternal / idleTimeoutMs /
      // maxResponseBytes / agent / errorClass) — it does NOT forward timeoutMs,
      // so a wall-clock cap passed there is silently dropped. Forward the idle
      // (zero-progress) bound via the honored idleTimeoutMs key, then enforce a
      // true wall-clock ceiling here with Promise.race so a server that keeps a
      // connection trickling forward progress on the encrypted route can't hang
      // a rename/init/finalize past the cap. Because _encryptedRequest serializes
      // every encrypted op through _encChain, an un-capped hang would also wedge
      // every subsequent encrypted op behind it; the wall-clock cap releases the
      // chain. The wrapper exposes no abort handle, so the underlying request is
      // left to its own idle cap — the caller is unblocked either way.
      resp = await this._withWallClock(
        enc.request({
          method:        method,
          path:          urlPath,
          body:          bodyData,
   ...
   * blamejs's downloadStream / uploadMultipartStream do not expose the
   * user-configurable bandwidth throttle (uploadBytesPerSec /
   * downloadBytesPerSec) those paths apply, nor the pre-opened-fd /
   * grew-or-shrank Content-Length guards on the upload — but they close
   * the same DNS-rebind TOCTOU window by resolving through the SSRF gate
   * (_pinnedServerLookup) and pinning the connect `lookup` to the
   * validated address set, so the SSRF/DNS-pin posture matches this path.
   */
