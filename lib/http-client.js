...
function spkiPin(certLike) {
  try {
    let x509;
    if (certLike instanceof nodeCrypto.X509Certificate) x509 = certLike;
    else if (Buffer.isBuffer(certLike)) x509 = new nodeCrypto.X509Certificate(certLike);
    else if (certLike && certLike.raw) x509 = new nodeCrypto.X509Certificate(certLike.raw);
    else return null;
...
class HttpClient {
  ...
  _rebuildAgent() {
    ...
    if (this._isHttps) {
      ...
      // Load-bearing, do NOT remove: create() routes through the framework
      // TLS posture, which sets options.groups to the same four groups in a
      // DIFFERENT preference order. On Node versions where OpenSSL honours
      // `groups` over `ecdhCurve`, this line is what keeps the advertised
      // order byte-identical to the server's group string (and both keys must
      // be set — Node versions differ on which one OpenSSL honours).
