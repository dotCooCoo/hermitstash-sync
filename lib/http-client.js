...
class HttpClient {
  ...
  reloadMtlsCerts() {
    ...
    try {
      if (this._config.mtls.cert) this._tlsOpts.cert = fs.readFileSync(this._config.mtls.cert);
      if (this._config.mtls.key)  this._tlsOpts.key  = fs.readFileSync(this._config.mtls.key);
      if (this._config.mtls.ca)   this._tlsOpts.ca   = fs.readFileSync(this._config.mtls.ca);
      ...
      try { this._agent.destroy(); } catch (_e) {}
      const Agent = this._isHttps ? https.Agent : http.Agent;
      this._agent = new Agent({
        ...this._tlsOpts,
        keepAlive: true,
