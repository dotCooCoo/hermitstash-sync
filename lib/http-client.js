...
// Loopback hosts (127.0.0.0/8, ::1, localhost) may legitimately speak plaintext
// http:// — a TLS-terminating sidecar on the same host, or a dev/test target.
// Every other host over http:// ships the Bearer API key and all sync metadata
// in cleartext with the post-quantum TLS floor absent. b.ssrfGuard.isLoopback
// covers the IP forms; it does not treat the "localhost" NAME as loopback, so
// special-case that.
function _isLoopbackHost(hostname) {
  if (typeof hostname !== 'string' || hostname === '') return false;
  // b.safeUrl.parse returns IPv6 hosts bracketed ("[::1]"); strip the brackets
  // before the loopback check, which expects the bare address.
