  ...
  async uploadFile(bundleId, relativePath, filePath) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = mod.request(reqOpts, res => {
        ...
        res.on('end', () => {
          // The body finished streaming before the server responded, so srcFd is
          // done being read. Close it here so EVERY response outcome (success,
          // error status, parse failure) releases it — autoClose is off, and the
          // resolve/reject paths below do not call cleanupSrc themselves.
          cleanupSrc();
