...
// Single home for the SPKI pin derivation (sha256/<base64> over SPKI DER) —
// shared with the checkServerIdentity pin verification so the value cert-info
// reports and the value the TLS layer verifies can never drift.
const { spkiPin } = require('./http-client');
...
function _redactedConfig() {
  ...
  for (const key of CONFIG_ALLOWLIST) {
    // Own-property read: keys come from the fixed allowlist, but cfg is
    // parsed operator input — inherited-member lookups must not resolve.
    if (Object.hasOwn(cfg, key) && cfg[key] !== undefined) out[key] = cfg[key];
  }
  ...
  // Tunnel mode is a non-secret boolean that changes how the TLS server
  // identity is verified (SAN skipped, SPKI pin enforced) — surface it for
  // support so a "why is it ignoring the hostname / refusing to start without a
  // pin" question is answerable from the bundle. Applied post-redact (it carries
  // no secret) alongside the pin COUNT above.
  redacted.tunnelMode = cfg.tunnelMode === true;
...
function _certInfo(cfg) {
  ...
  try {
    ...
    // that reuses the same keypair preserves the pin. NOTE: this is the
    // pin of THIS cert (the mTLS client cert when called on cfg.mtls.cert)
    // — config.pinnedServerSpki must be the SERVER cert's pin; see the
    // README's openssl recipe against server.crt for that value.
    const certSpkiPin = spkiPin(x);
...
function _logFiles() {
  ...
  try {
    ...
    for (const name of entries) {
      ...
      try {
        // lstat (not stat) so a symlink planted in the log dir under a
        // rotation-shaped name is NOT classified as a file and is skipped here
        // — it must never be enumerated, sized, or read as if it were a log.
...
// Secret literals present inline in the operator's config.json — a raw apiKey /
// apiKeyRef / token / password, when one is stored there. These are scrubbed
// verbatim from log content at bundle time.
//
// Scope, stated honestly: this client normally keeps its bearer credential in
// the OS keychain (or the sibling credentials file), so config.json holds only
// a NON-secret reference (apiKeyRef 'keychain:...'), not the secret — and this
// module deliberately never reads the credential store to build a support
// bundle. So the config-literal list below covers only the uncommon
// inline-pasted secret. The resolved bearer credential, if it ever lands in a
// free-form log message (e.g. an error echoing an Authorization header), is
// caught by SHAPE in _scrubBearerCredentials rather than by value here; and the
// primary control remains the framework's emit-time field-name redaction
// (b.redact masks an authorization/token/apikey FIELD before it reaches disk).
...
// Expand each known config-literal secret into the byte-forms it can take in a
// log file so the scrub matches regardless of encoding. A JSON log record is
// re-serialized by _redactLogLine (JSON.stringify), which RE-ESCAPES every
// string value — so a secret containing a JSON-escapable byte (" \ control)
// appears escaped there (`ab\"cd`) while the config value was parse-DECODED
// (`ab"cd`). A plain `redacted.includes(decoded)` then misses it. Scrub against
// BOTH the decoded value (matches a raw / non-JSON line) AND its JSON-escaped
// inner form (matches a re-stringified JSON record). Each literal is first
// normalized to its decoded value so a literal harvested from an UNPARSEABLE
// config's raw bytes (which still carries its on-disk escapes) and one read
// from a parsed config converge on the same canonical pair.
function _expandSecretForms(literals) {
  const forms = new Set();
  for (const lit of literals) {
    if (typeof lit !== 'string' || lit.length < MIN_SECRET_LITERAL_LEN) continue;
    let decoded = lit;
    try {
      // Decode the literal as a JSON string body. A value read from parsed
      // config is already decoded (this is a no-op / throws harmlessly); a
      // raw-harvested value carries its on-disk escapes and decodes here.
      const d = b.safeJson.parse('"' + lit + '"', { maxBytes: C.BYTES.mib(1) });
      if (typeof d === 'string' && d.length > 0) decoded = d;
    } catch { /* allow:silent-catch — keep the raw literal when it isn't a valid JSON string body */ }
    for (const form of [decoded, lit, JSON.stringify(decoded).slice(1, -1)]) {
      if (typeof form === 'string' && form.length >= MIN_SECRET_LITERAL_LEN) forms.add(form);
    }
  }
  return [...forms];
}

// Shape-based scrub for a bearer / authorization credential embedded in a
// free-form log message. The config-literal scrub above only catches a secret
// stored inline in config.json; this client's real bearer credential lives in
// the OS keychain (or the sibling credentials file) and is never read here, and
// b.redact's field-name pass masks an `authorization` KEY but not the same
// token when it sits inside a `message` STRING (an error echoing a request
// header). Redact whatever follows the `Bearer ` scheme keyword by shape,
// regardless of its literal value, so the resolved credential can't ride a
// free-form line into the bundle. This only ADDS masking — it weakens nothing.
function _scrubBearerCredentials(text) {
  if (typeof text !== 'string' || text.length === 0) return text;
  // `Bearer <token>` (bare or after `Authorization:`). Token is the API-key /
  // JWT charset; the eight-char floor avoids masking a short word ("Bearer
  // bonds"). Bounded, linear; the caller already byte-caps the line length.
  // Case-insensitive scheme match: RFC 9110 auth schemes are matched
  // case-insensitively, so a `BEARER`/`bEaReR` variant carries the same
  // credential and must scrub the same way.
  return text.replace(/\b(bearer)\s+([A-Za-z0-9._~+\/=-]{8,})/gi, '$1 [REDACTED]');
}

// Re-redact a UTF-8 log line. The framework redacts at EMIT time, but a value
// already on disk was written before any rule that would catch it (or by an
// older binary), so we re-run redaction over the bytes at bundle time. Each
// line is a JSON record ({ts,level,message,meta}); we re-redact the structured
// object with b.redact.redact (FIELD-based — catches a sensitive-named meta
// field) and the free-form `message` value with b.redact.redactText
// (word-boundary FREE-TEXT — catches a JWT / AWS key / URL-userinfo password /
// bearer / key=secret embedded MID-message, which b.redact's whole-value
// detectors miss). A line that doesn't parse as JSON falls back to redactText
// over the whole free-text line. Then scrub any known config secret literal and
// any bearer-shaped credential as a whole-string backstop.
function _redactLogLine(line, secretLiterals) {
  ...
  if (obj && typeof obj === 'object') {
    ...
    if (r && typeof r.message === 'string') r.message = b.redact.redactText(r.message);
    redacted = JSON.stringify(r);
  } else {
    redacted = b.redact.redactText(line);
  }
  for (const lit of secretLiterals) {
    if (redacted.includes(lit)) redacted = redacted.split(lit).join('[REDACTED]');
  }
  // Shape backstop for a resolved bearer/authorization credential that reached
  // a free-form message before any field rule could mask it (the keychain-
  // stored credential is never in secretLiterals). See _scrubBearerCredentials.
  redacted = _scrubBearerCredentials(redacted);
  return redacted;
}
...
// Reduce any absolute daemon-dir path embedded in a diagnostic message to its
// basename so the OS username / home-dir layout the bundle contract lists as
// out-of-scope never rides an error string into the handoff .zip. A corrupt-DB
// / open error surfaced from below (node:sqlite / b.localDb under
// recovery:'refuse') embeds the full STATE_DB_FILE path in its message; the
// _absent branch already reduces the path it reports, but the error string is
// unstructured. Strip the longest (most specific) path first so the shorter
// CONFIG_DIR replacement doesn't leave a dangling suffix.
function _scrubAbsolutePaths(msg) {
  if (typeof msg !== 'string' || msg.length === 0) return msg;
  let out = msg;
  for (const abs of [STATE_DB_FILE, CONFIG_DIR]) {
    if (typeof abs === 'string' && abs.length > 0) {
      out = out.split(abs).join(nodePath.basename(abs));
    }
  }
  return out;
}
...
function _stateDbSchema() {
  ...
  let opened = false;
  try {
    if (!nodeFs.existsSync(STATE_DB_FILE)) return { _absent: true, path: nodePath.basename(STATE_DB_FILE) };
    try { stateDb.db(); } catch { stateDb.open(undefined, { recovery: 'refuse' }); opened = true; }
    return stateDb.dumpSchema();
  } catch (err) {
    return { error: _scrubAbsolutePaths(err && err.message) };
  } finally {
...
function buildBundle(outPath) {
  ...
  // Expand each known literal into the two byte-forms it can take in a log file
  // — the decoded value (a raw / non-JSON line) and its JSON-escaped inner form
  // (a JSON record that _redactLogLine re-stringifies) — so a secret with a
  // JSON-escapable character (" \ control) is scrubbed in both.
  const secretLiterals = _expandSecretForms(_knownSecretLiterals());
  const logCap = C.BYTES.mib(50);
  for (const entry of _logFiles()) {
    if (entry.size > logCap) continue; // skip pathological log files
    try {
      // Symlink-refusing read, matching every other read in this module
      // (_readJsonIfPresent, _certInfo, _harvestRawSecretLiterals): a bare
      // readFileSync follows a symlink (CWE-59), so a link planted in the log
      // dir under a rotation-shaped name would read its target into the bundle,
      // defeating the "only log files" contract (redaction only catches KNOWN
      // secret shapes). refuseSymlink lstat-refuses + pins the fd; maxBytes caps
      // a TOCTOU grow past the size pre-check above. No encoding → raw Buffer
      // for the gzip/UTF-8 redaction step below.
      const raw = b.atomicFile.fdSafeReadSync(entry.abs, { refuseSymlink: true, maxBytes: logCap });
...
// Closure-internal helpers exposed for unit coverage (see tests/test-fix-*.js).
// Not part of the public CLI surface — the daemon and cli.js call only
// buildBundle / defaultOutPath.
module.exports._internals = {
  _stateDbSchema,
  _scrubAbsolutePaths,
  _knownSecretLiterals,
  _expandSecretForms,
  _redactLogLine,
  _scrubBearerCredentials,
};
