...
class Watcher extends EventEmitter {
  ...
  _processChange(relativePath) {
    ...
    try {
      if (fs.existsSync(fullPath)) {
        // H2: Use lstatSync to detect symlinks — skip them
        const stat = fs.lstatSync(fullPath);
        if (stat.isSymbolicLink()) return; // Never follow symlinks
  ...
  /**
   * L2: Public API for ignore-pattern checking (used by sync-engine).
   */
