...
// Mirror b.watcher.create's _compileIgnore caps (vendor/blamejs/lib/watcher.js
// MAX_IGNORE_PATTERN_LEN / MAX_IGNORE_STAR_COUNT). b.watcher does not export
// them, so pin them here. A forwarded ignore pattern past either cap makes
// b.watcher.create throw `watcher/bad-ignore` SYNCHRONOUSLY — which would
// abort the initial (unguarded) start() or drive the SIGHUP recovery loop to a
// permanent 'degraded' state. Forwarding is only the walk-prune optimization
// (the hook-side _shouldSync still enforces the FULL ignore set), so an
// over-cap pattern is dropped from the forwarded subset rather than allowed to
// kill the whole watcher. Length is measured in UTF-16 code units to match
// b.watcher's own `p.length` check (NOT Buffer.byteLength).
const WATCHER_IGNORE_MAX_LEN = 256;   // allow:raw-byte-literal — mirrors b.watcher MAX_IGNORE_PATTERN_LEN (code units, not bytes)
const WATCHER_IGNORE_MAX_STARS = 16;  // allow:raw-byte-literal — mirrors b.watcher MAX_IGNORE_STAR_COUNT (count, not bytes)
...
function _dirPrefixIgnores(patterns, caseFolds) {
  ...
  for (const p of patterns) {
    ...
    // On a case-folding volume ALSO forward the lowercase form: b.watcher's
    // poll-prune matches case-sensitively, so an on-disk 'Node_Modules' vs a
    // forwarded 'node_modules/**' would NOT be pruned and the walk would stat
    // the whole subtree toward pollMaxFiles → watcher/poll-overflow → the
    // watcher goes 'degraded'. The hook-side _shouldSync folds case and stays
    // authoritative for correctness; this only widens the poll-prune so the
    // primary overflow defence isn't defeated by a casing mismatch. (The full
    // fix is an upstream b.watcher case-fold ignore option.)
    const forms = [p, p.normalize('NFC'), p.normalize('NFD')];
    if (caseFolds) forms.push(p.toLowerCase(), p.normalize('NFC').toLowerCase());
...
class Watcher extends EventEmitter {
  constructor(syncFolder, ignorePatterns = [], includePatterns = [], caseFolds = false) {
    ...
    this._dirIgnores = _dirPrefixIgnores(ignorePatterns, caseFolds); // last-forwarded dir/** subset
  }
  ...
  _recreate() {
    ...
    try {
      ...
      this._dirIgnores = _dirPrefixIgnores(this._ignorePatterns, this._caseFolds);
  ...
  updatePatterns(ignorePatterns, includePatterns) {
    this._ignorePatterns = ignorePatterns || [];
    this._includePatterns = includePatterns || [];
    var nextDirIgnores = _dirPrefixIgnores(this._ignorePatterns, this._caseFolds);
    ...
    //     a full daemon restart. start() resets _recoveryAttempts, giving the
    //     freshly-narrowed config a clean retry budget.
    if (this._handle) {
