...
class Watcher extends EventEmitter {
  ...
  _buildOpts() {
    ...
    var opts = {
      ...
      onChange: ev => {
        ...
        this.emit('change', {
          type: 'change',
          relativePath: _nfc(ev.relativePath.split(nodePath.sep).join('/')),
  ...
  updatePatterns(ignorePatterns, includePatterns) {
    ...
    if (changed && this._handle && !this._stopped) {
      ...
      // since a bundleId-only resync skips the local-upload scan. Gated on
      // `changed` so a no-op pattern edit doesn't trigger a needless walk.
      this.emit('rescan');
    }
  }
