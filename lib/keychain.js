...
function _readFileFallback() {
  ...
  // TOCTOU-safe read of the secret (CWE-367): open read-only and bind the
  // returned bytes to the held inode, refusing a symlink source and any inode
  // swap (the strongest posture, for operator-writable source paths). We
  // exclusively own CREDENTIALS_FILE in CONFIG_DIR — it is only ever written
  // via b.atomicFile.writeSync — so a symlink there is never legitimate; it is
  // exactly the swap an attacker with parent-dir write would plant between the
  // permission check above and this read. The 64 KiB cap is far above any real
  // API key / sync token but refuses a bloated file. errorFor maps each refusal
  // to an actionable KeychainError instead of a raw atomic-file error.
  return b.atomicFile.fdSafeReadSync(CREDENTIALS_FILE, {
    refuseSymlink: true,
    inodeCheck:    true,
