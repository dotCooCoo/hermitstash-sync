...
async function store(apiKey, ref) {
  ...
  if (sel.mode === 'keychain') {
    try {
      ...
      // The native backend was selected and present but the store FAILED (e.g.
      // the OS keychain is locked, or the D-Bus session is unavailable). This is
      // distinct from a genuinely-absent backend, which resolves to
      // backend:'file' above without throwing and is the normal headless path.
      // Surface one actionable line — no key material, only the framework error
      // code and the fallback path — so the operator knows the secret landed in
      // the 0o600 plaintext fallback and how to migrate it back. Then fall
      // through to the file fallback so enrollment still succeeds.
