...
async function store(apiKey, ref) {
  ...
  if (sel.mode === 'keychain') {
    try {
      ...
      // Two distinct cases land here, and they need different messages:
      //   (1) No native credential store is present (headless Linux without
      //       libsecret, most containers). blamejs picks the file backend and
      //       then throws `keychain/bad-fallback-file` here because we supply no
      //       fallbackFile (we own the 0o600 on-disk fallback ourselves). This
      //       is the NORMAL, expected path — telling the operator to "unlock the
      //       keychain" is impossible and misleading. Emit a calm info line.
      //   (2) A native store IS present but the store FAILED (locked login
      //       keychain, dead D-Bus). That carries a different code and IS worth
      //       an actionable warning.
      // Either way fall through to the 0o600 file write so enrollment succeeds.
      // No key material is ever logged — only the framework code + the path.
      if (err && err.code === 'keychain/bad-fallback-file') {
        log.info('No OS credential store detected — stored the API key in the 0o600 fallback file at ' + CREDENTIALS_FILE + '.');
      } else {
        log.warn('OS keychain store failed (' + ((err && err.code) || 'unavailable') +
          ') — wrote the API key to the 0o600 fallback file at ' + CREDENTIALS_FILE +
          '. Unlock the keychain and re-run "hermitstash-sync init" to migrate it natively.');
      }
    }
  }
...
async function retrieveDetailed(ref) {
  ...
  // Attempt the file fallback directly rather than gating on existsSync: a
  // concurrent unlink (a native-keychain re-enroll removing the file) between
  // an existsSync check and the open would race the reader to a spurious
  // KeychainError. Treat a genuinely-missing file as "no file fallback" and
  // fall through to the disambiguation below; any other read failure (symlink,
  // over-size, TOCTOU swap) is a real error and propagates.
  try {
    return { found: true, apiKey: _readFileFallback() };
  } catch (err) {
    const code = (err && err.code) || '';
    const missing = code === 'ENOENT' || /enoent|not-found/i.test(String(code));
