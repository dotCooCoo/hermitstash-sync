...
async function exchangeEnrollmentCode(serverUrl, code) {
  ...
  if (resp.statusCode !== 200 || !data.success) {
    throw new Error(data.detail || data.error || `Server returned status ${resp.statusCode}`);
  }
  return data;
}
...
async function cmdStart(args) {
  ...
  try {
    ...
    // Durable auto-update probation confirmation. Reaching a running engine is
    // the health signal probation exists to confirm, so record it on disk —
    // otherwise an unrelated restart (systemd Restart=, reboot, operator
    // stop/start) during the probation window would leave the just-installed
    // binary looking like a crash and trigger a spurious downgrade on the next
    // boot. No-op unless a fresh self-update is on probation.
...
module.exports = { run, buildEnrollmentAgent };
