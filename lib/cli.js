...
function buildEnrollmentAgent() {
  ...
  // future vendor bump narrows the default preference. The groups mutation is
  // load-bearing: create()'s TLS posture sets options.groups in a different
  // preference order, and both keys must be set (Node versions differ on
  // which one OpenSSL honours). minVersion TLSv1.3 is pinned by create().
...
function saveMtlsCerts(enrollData) {
  ...
  // b.atomicFile.writeSync: write-temp + fsync + rename + parent-dir-fsync, so
  // EACH file lands atomically — a crash mid-write of one file never leaves a
  // half-written cert/key. The three writes are NOT a single transaction,
  // though: a rare failure after the cert is written but before the key (e.g. a
  // disk error between two renames) would leave the new cert beside the old
  // key, which fails the TLS handshake on the next start. That torn state is
  // recoverable by re-running init/repair, which rewrites all three.
...
/**
 * Resolve the server SPKI pin(s) that tunnel mode will verify against.
 *
 * PREFERS a pin supplied out-of-band (HERMITSTASH_PINNED_SERVER_SPKI): that
 * value reached this machine over a channel the network attacker does not
 * control, so it binds the server's identity without trusting whatever answers
 * at setup time. No probe is performed in that case.
 *
 * Only with no supplied pin do we fall back to capturing the leaf we see
 * (trust-on-first-use), and only where a human can confirm it. The reason is
 * specific to tunnel mode: an attacker on the network path at exactly this
 * moment can answer the probe with a certificate they own from any public CA,
 * and it is their SPKI that gets pinned. Because tunnel mode also skips the
 * hostname check, every later connection would accept them — the chain
 * validates, the hostname is not consulted, and the pin is theirs — so the
 * interception persists silently instead of breaking visibly the way it would
 * with the hostname check on.
 *
 * An unattended setup therefore does NOT capture: with nobody to compare the
 * observed pin against the real server, any "proceed" flag would only assert
 * "pin whatever answers" — it names no value, so it cannot tell the server from
 * an interceptor, and a stray value left in a profile or unit file would also
 * silence the interactive confirmation. Headless setup is pointed at the
 * out-of-band pin instead, which is the only form that binds an actual value.
 */
...
async function cmdStart(args) {
  ...
  // Boot-time NTP drift gate. Runs BEFORE the auto-update rollback check
  // below, because that check makes a Date.now()-based, irreversible decision
  // and must not act on an unvalidated clock. Two paths in the daemon depend
  // on the wall clock being roughly correct: the auto-update probation
  ...
  // far enough to corrupt those decisions silently. Validating the clock
  // first means a fatally-skewed boot refuses to start before the
  // destructive rollback/probation branch can mis-fire on a bad Date.now().
  //
