...
async function exchangeEnrollmentCode(serverUrl, code) {
  ...
  try {
    resp = await b.httpClient.request({
      ...
      agent: new httpsLib.Agent({ rejectUnauthorized: false }),
...
function saveMtlsCerts(enrollData) {
  ...
  b.atomicFile.writeSync(cert, enrollData.clientCert, { mode: 0o644 });
  b.atomicFile.writeSync(key,  enrollData.clientKey,  { mode: 0o600 });
  b.atomicFile.writeSync(ca,   enrollData.caCert,     { mode: 0o644 });
  return { cert, key, ca };
}
...
async function cmdStart(args) {
  ...
  daemon.installSignalHandlers(
    async () => {
      metrics.stopWriter();
      // Final snapshot on shutdown so the stats file reflects the
      // last-known state instead of whatever the writer flushed up to
      // STATS_WRITE_INTERVAL_MS ago.
      metrics.writeSnapshot();
      await engine.stop();
      await log.close();
    },
    () => engine.resync(),
...
function cmdStats(args = []) {
  ...
  // STATS_WRITE_INTERVAL_MS. If it's missing or stale, fall back to the
  // state DB so `stats` works while the daemon is stopped — it just
  // can't surface in-process counters then.
  let snap = null;
  if (fs.existsSync(STATS_FILE)) {
    try {
      snap = b.safeJson.parse(fs.readFileSync(STATS_FILE, 'utf8'), { maxBytes: C.BYTES.kib(64) });
    } catch (err) {
      console.error(`Could not parse ${STATS_FILE}: ${err.message}`);
    }
  }
  ...
  if (asProm) {
    // Prometheus exposition is only available in-process — the stats
    // file ships JSON snapshots. Print a synthetic exposition from the
    // shadow values so external scrapers can still ingest the file.
    if (!snap) {
    ...
    for (const [k, v] of Object.entries(snap.counters || {})) {
      lines.push(`# TYPE hermitstash_sync_${k} counter`);
      lines.push(`hermitstash_sync_${k} ${v}`);
    }
    for (const [k, v] of Object.entries(snap.gauges || {})) {
      if (typeof v === 'number') {
        lines.push(`# TYPE hermitstash_sync_${k} gauge`);
        lines.push(`hermitstash_sync_${k} ${v}`);
      }
    }
  ...
  if (snap) {
    ...
    const stale = !running || ageSec > _SEC_PER_MIN;
    console.log(`Snapshot:  ${snap.writtenAt}${stale ? '  (stale)' : ''}`);
    console.log(`Started:   ${snap.startedAt || '—'}`);
    console.log(`Uptime:    ${_fmtDuration(snap.uptimeSec)}`);
    console.log('');
    console.log(`State:     ${snap.gauges.sync_state}`);
    console.log(`Files:     ${snap.gauges.file_count}`);
    console.log(`Active:    ${snap.gauges.active_ops} operation(s)`);
    console.log(`Last seq:  ${snap.gauges.last_seq}`);
    console.log(`Circuit:   upload=${snap.gauges.circuit_state_upload}`);
    console.log('');
    console.log('Counters since start:');
    console.log(`  Uploads:        ok=${snap.counters.uploads_ok} error=${snap.counters.uploads_error} retries=${snap.counters.upload_retries}`);
    console.log(`  Downloads:      ok=${snap.counters.downloads_ok} error=${snap.counters.downloads_error}`);
    console.log(`  WS reconnects:  ${snap.counters.ws_reconnects}`);
    console.log(`  Auth errors:    ${snap.counters.ws_auth_errors}`);
    console.log(`  Circuit opens:  upload=${snap.counters.circuit_opens_upload}`);
  } else if (running) {
...
async function cmdRepair() {
  ...
  const cfg = b.safeJson.parse(fs.readFileSync(CONFIG_FILE, 'utf8'), { maxBytes: C.BYTES.mib(1) });
  ...
  try {
    ...
    const mtls = saveMtlsCerts(ed);
    if (mtls) console.log('  mTLS certificates updated at ' + path.dirname(mtls.cert));
...
Environment overrides:
  ...
  HERMITSTASH_WATCHER_MODE=poll   Use poll-mode file watcher (Docker bind-mounts)
