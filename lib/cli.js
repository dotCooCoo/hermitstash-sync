...
const { VERSION, LOG_FILE, CONFIG_DIR, CONFIG_FILE } = require('./constants');
...
async function exchangeEnrollmentCode(serverUrl, code) {
  ...
  const { parseHttpStatusFromMessage } = require('./http-client');

  const url = new URL(serverUrl);
  const enrollUrl = `${url.protocol}//${url.host}/sync/enroll`;
  ...
  try {
    ...
    if (err && err.code === 'HTTP_ERROR') {
      const httpErr = parseHttpStatusFromMessage(err.message);
      if (httpErr) {
        const parsed = bSafeJson.parseOrDefault(httpErr.body, null, { maxBytes: 256 * 1024 });
        const errMsg = (parsed && parsed.error) || `Server returned status ${httpErr.code}`;
...
function saveMtlsCerts(enrollData) {
  ...
  const bAtomicFile = require('../vendor/blamejs/lib/atomic-file');
  ...
  bAtomicFile.writeSync(cert, enrollData.clientCert, { mode: 0o644 });
  bAtomicFile.writeSync(key,  enrollData.clientKey,  { mode: 0o600 });
  bAtomicFile.writeSync(ca,   enrollData.caCert,     { mode: 0o644 });
  return { cert, key, ca };
}
