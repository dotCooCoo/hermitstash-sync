...
function cmdStats(args = []) {
  ...
  if (asProm) {
    ...
    if (nodeFs.existsSync(STATS_PROM_FILE)) {
      try {
        process.stdout.write(nodeFs.readFileSync(STATS_PROM_FILE));
        return;
      } catch (err) {
...
async function cmdRepair() {
  ...
  const cfg = loadConfigOrDie();
  // Validate + normalize the configured server URL before display (and before the
  // enrollment exchange below uses it), and show the reconstructed origin —
  // mirroring the wire layer (lib/ws-client.js / lib/http-client.js), which routes
  // config.server through b.safeUrl.parse rather than emitting the raw config
  // string. A malformed server in config.json now surfaces here with a clear URL
  // error instead of being printed and then failing the enrollment call.
  const serverUrl = b.safeUrl.parse(cfg.server, { allowedProtocols: ['http:', 'https:'] });
  console.log('Repair — re-provision mTLS certificates from enrollment code');
  console.log(`Server: ${serverUrl.protocol}//${serverUrl.host}`);
  console.log('');
...
module.exports = { run, buildEnrollmentAgent, exchangeEnrollmentCode };
