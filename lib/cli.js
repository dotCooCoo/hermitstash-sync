...
async function exchangeEnrollmentCode(serverUrl, code) {
  ...
  const bFrameworkError = require('../vendor/blamejs/lib/framework-error');
  ...
  const EnrollHttpError = bFrameworkError.defineClass('EnrollHttpError', { withStatusCode: true });
  ...
  try {
    ...
    if (err instanceof EnrollHttpError && err.statusCode) {
      ...
      const parsed = bSafeJson.parseOrDefault(errBody, null, { maxBytes: 256 * 1024 });
