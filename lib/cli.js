...
async function cmdStop() {
  ...
  // On win32, process.kill(pid, 'SIGTERM') is TerminateProcess — the daemon's
  // graceful teardown (and its updater.markGracefulShutdown() stamp) never
  // runs. Stamp on the daemon's behalf so an operator stop during the
  // auto-update probation window isn't misread as a crash on the next start
  // (which would roll a healthy update back). markGracefulShutdown is
  // version-guarded and idempotent. Gate strictly on stopped === true: the
  // no-pidfile/stale paths mean the daemon was ALREADY dead — possibly a
  // genuine probation crash — and stamping those would defeat the rollback
  // safety net. POSIX stays untouched: the daemon stamps for itself, and a
  // CLI-side stamp there would mask the SIGKILL-escalation (hung daemon)
  // case where rollback is intentional.
  if (stopped === true && process.platform === 'win32') {
    updater.markGracefulShutdown();
    console.log('Note: Windows stop is forceful (no graceful drain); an in-flight upload may have been cut and will resume on the next start.');
  }
}
