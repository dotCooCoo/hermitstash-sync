...
const { VERSION, LOG_FILE, CONFIG_DIR, CONFIG_FILE, STATS_FILE, TLS_GROUPS } = require('./constants');
...
// Flush the async log-stream file sink before a terminal DAEMON exit so the
// final error line reaches the rotated on-disk log — a background daemon's only
// post-mortem sink (stdout/journal aside). Bounded (2s) so a stuck sink can't
// hang the exit. Used by the daemon-mode start / auth-error exit paths; die()
// stays synchronous for CLI validation where stderr is a live terminal.
async function _drainLogsBeforeExit() {
  try { await b.safeAsync.withTimeout(log.close(), C.TIME.seconds(2), { name: 'exit-log-drain' }); }
  catch { /* allow:silent-catch — best-effort final flush before a terminal exit */ }
}

// A server URL as a safe display label, before it reaches any log sink. The
// parse refuses a credential in the authority outright, and the label is
// rebuilt from the parsed origin rather than echoing the configured string, so
// nothing supplied by the environment is ever logged verbatim. A malformed
// value yields an actionable placeholder rather than throwing, so a status line
// still renders. Shared, so the masking lives in one place.
...
function buildEnrollmentAgent() {
  ...
  // future vendor bump narrows the default preference. minVersion TLSv1.3 is
  // pinned by create(). `ecdhCurve` is the only key set: node:tls reads no
  // `groups` option, so assigning one is accepted and discarded and would
  // read as a second handle on a preference it cannot actually change.
  const agent = b.pqcAgent.create({
    keepAlive: false,
    ecdhCurve: TLS_GROUPS,
    allowOperatorGroups: true,
  });
  agent.options.rejectUnauthorized = false; // allow:tls-verify-disabled — first-contact enrollment before any CA is provisioned; the server-issued client cert + CA pin take over for every subsequent request
  return agent;
}
...
async function exchangeEnrollmentCode(serverUrl, code) {
  ...
  // Refuse a non-loopback plaintext http:// target BEFORE the exchange. This
  // POST sends the enrollment code and receives the API key + mTLS PRIVATE KEY
  // — over http:// to a non-loopback host they travel in cleartext with no
  // PQC-TLS protection. config.validate enforces the same rule, but it only
  // runs when the daemon later starts, long after enrollment has already
  // leaked the credentials. A loopback target (a TLS-terminating sidecar) is
  // allowed, and HERMITSTASH_ALLOW_PLAINTEXT is the deliberate opt-out — same
  // policy and env var as config.validate.
  if (parsedUrl.protocol === 'http:' && !config.isLoopbackHost(parsedUrl.hostname)) {
    const allowPlain = b.safeEnv.readVar('HERMITSTASH_ALLOW_PLAINTEXT', { type: 'string', default: '' });
    if (!(allowPlain === '1' || allowPlain === 'true' || allowPlain === 'yes')) {
      throw new Error(
        `Refusing to enroll over plaintext http:// to a non-loopback host — the enrollment ` +
        `exchange would send your enrollment code and receive the API key and mTLS private key ` +
        `in cleartext. Use https:// (the HermitStash server requires PQC-mTLS), or set ` +
...
function saveMtlsCerts(enrollData) {
  // Distinguish three cases instead of an all-or-nothing return: no cert fields
  // at all is an intentional API-key-only enrollment (return null); a PARTIAL
  // set (one or two of the three) is a server/protocol error — surface it
  // rather than silently writing nothing and reporting success, which would
  // leave any prior certs stale; all three present is a normal mTLS enrollment.
  const present = [enrollData.clientCert, enrollData.clientKey, enrollData.caCert].filter(Boolean).length;
  if (present === 0) return null;
  if (present < 3) {
    throw new Error(`Server returned an incomplete mTLS certificate set (${present} of 3 fields present) — cannot complete enrollment. Retry once the server issues the client certificate, key, and CA together.`);
  }
  ...
  // Each file lands atomically, but the three are not one transaction: a
  // failure between two renames leaves a new certificate beside an old key,
  // which fails the next handshake. Re-running init or repair rewrites all
  // three and recovers it.
  //
  // Each PEM ends with a newline so an operator appending extra trust anchors
  // gets a parseable bundle — without it the appended certificate begins on the
  // same line as the END marker and the whole file is rejected.
 ...
 */
async function saveSyncConfig({ server, bundleId, shareId, syncFolder, apiKey, mtls, autoUpdate, tunnelMode, pinnedServerSpki }) {
  ...
  // Tunnel mode: persist the trust-on-first-use server SPKI pin captured at
  // enrollment AND the tunnelMode switch together, so config.validate (which
  // refuses tunnelMode without a pin) always sees a consistent pair.
  if (Array.isArray(pinnedServerSpki) && pinnedServerSpki.length > 0) cfg.pinnedServerSpki = pinnedServerSpki;
  if (tunnelMode === true) cfg.tunnelMode = true;
...
/**
 * Tunnel-mode setup: TLS-probe the server's leaf cert and return its SPKI pin
 * (trust-on-first-use). The pin is what a relaxed-hostname tunnel connection
 * verifies against, so it MUST be captured before the config is written — tunnel
 * mode cannot be set up (and would refuse to start) without it. Throws with an
 * actionable message on any probe failure.
 */
async function captureTunnelPin(server) {
  try {
    return await HttpClient.probeServerLeafSpki(server);
  } catch (err) {
    throw new Error(
      'tunnel mode requested but capturing the server SPKI pin failed: ' + (err && err.message) +
      '. Tunnel mode reaches the server over its real https:// port inside the tunnel and pins ' +
      'its leaf certificate — make sure the URL is the tunnel https:// address, then retry.');
  }
}

// Pins the operator supplied out-of-band, via HERMITSTASH_PINNED_SERVER_SPKI
// (same comma-separated shape config.js's env overlay accepts).
function _suppliedPins() {
  return b.safeEnv.readVar('HERMITSTASH_PINNED_SERVER_SPKI', { type: 'string', default: '' })
    .split(',').map(s => s.trim()).filter(Boolean);
}

/**
 * The server pin tunnel mode verifies against.
 *
 * An out-of-band pin is preferred and skips the probe entirely: it reached this
 * machine over a channel the network attacker does not control, so it binds the
 * server's identity without trusting whatever answers at setup time.
 *
 * Only without one does this capture the leaf it sees, and only where a human
 * can confirm it. Tunnel mode also skips the hostname check, so an attacker who
 * answers the probe with a certificate they own from any public CA gets their
 * own SPKI pinned — and every later connection accepts them, because the chain
 * validates and the hostname is never consulted. The interception persists
 * silently rather than breaking visibly.
 *
 * An unattended setup therefore never captures. With nobody to compare the
 * observed pin against the real server, a "proceed" flag would only mean "pin
 * whatever answers": it names no value, so it cannot tell the server from an
 * interceptor, and one left behind in a profile would also silence the
 * confirmation. Headless setup is pointed at the out-of-band pin instead.
 */
async function resolveTunnelPins(server, opts) {
  const interactive = !!(opts && opts.interactive);
  const supplied = _suppliedPins();
  if (supplied.length) {
    console.log(`  Using ${supplied.length} server SPKI pin(s) supplied out-of-band — no capture needed`);
    return supplied;
  }

  if (!interactive) {
    throw new Error(
      'tunnel mode needs the server\'s SPKI pin. Because tunnel mode does not check the hostname, that ' +
      'pin is what binds the server\'s identity, and an unattended setup cannot confirm that the ' +
      'certificate answering right now is really your server. Read the pin off the server and pass it ' +
      'in HERMITSTASH_PINNED_SERVER_SPKI, then re-run. (Interactive "hermitstash-sync init --tunnel" ' +
      'will show you the pin it observes and ask you to confirm it.)');
  }

  console.log('\nCapturing the server SPKI pin for tunnel mode...');
  const pin = await captureTunnelPin(server);
  console.log('  Server SPKI pin: ' + pin);

  // Fresh readline: the enrollment branch closes the setup prompt before the
  // exchange, so we cannot reuse it here.
  const rl2 = nodeReadline.createInterface({ input: process.stdin, output: process.stdout });
  let answer;
  try {
    answer = await new Promise(resolve => rl2.question(
      '  Does this pin match the server\'s certificate? Confirm out-of-band before accepting. (y/N): ',
      resolve));
  } finally {
    try { rl2.close(); } catch (_e) { /* allow:silent-catch — prompt teardown; close is idempotent */ }
  }
  if (String(answer || '').trim().toLowerCase() !== 'y') {
    throw new Error(
      'tunnel-mode setup cancelled — the server SPKI pin was not confirmed. Obtain the pin from the ' +
      'server out-of-band and pass it via HERMITSTASH_PINNED_SERVER_SPKI, then re-run.');
  }
  return [pin];
}

// True when the operator asked for tunnel mode: `--tunnel` on the CLI, or the
// HERMITSTASH_TUNNEL_MODE env var set truthy (matching config.js's _bool
// overlay: any non-blank value that isn't 0/false/no/off).
function _wantTunnel(args) {
  if (Array.isArray(args) && args.includes('--tunnel')) return true;
  const raw = b.safeEnv.readVar('HERMITSTASH_TUNNEL_MODE', { type: 'string', default: '' }).trim();
...
async function cmdInit(args = []) {
  ...
  try {
    ...
    if (authMethod === '2') {
      // Trim and require a non-empty key: a pasted key routinely carries
      // trailing whitespace/newline, and an empty entry would otherwise be
      // stored and fail opaquely at the first authenticated request.
      apiKey = (await ask('API key or sync token: ')).trim();
      if (!apiKey) { rl.close(); die('API key required.'); }
      bundleId = (await ask('Bundle ID: ')).trim();
      shareId = (await ask('Share ID (for metadata access): ')).trim();
      ...
      // Guard an empty code locally instead of POSTing {code:''} and surfacing
      // a generic server-side 'Enrollment failed'. Mirrors cmdRepair's
      // 'Code required.' short-circuit. die() -> process.exit(1) terminates
      // before rl.close(), which is harmless (the process is exiting).
      if (!code.trim()) die('Enrollment code required.');
      rl.close();

      console.log('\nExchanging enrollment code...');
      const ed = await exchangeEnrollmentCode(server, code);
      // Guard the credential before it reaches keychain.store: a success
      // response with no api key (e.g. a certificate-reissue code sent to the
      // enrollment path) would otherwise store an undefined/empty key and fail
      // opaquely later. Mirror cmdRepair, which handles the cert-only case.
      if (!ed.apiKey || typeof ed.apiKey !== 'string' || !ed.apiKey.trim()) {
        die('Enrollment succeeded but the server returned no API key. If you were issued a certificate-reissue code, run "hermitstash-sync repair" instead of "init".');
      }
      apiKey = ed.apiKey;
    ...
    let tunnelMode, pinnedServerSpki;
    if (_wantTunnel(args)) {
      pinnedServerSpki = await resolveTunnelPins(server, { interactive: true });
      tunnelMode = true;
    }
    const { resolvedFolder, storage } = await saveSyncConfig({
      server, bundleId, shareId, syncFolder, apiKey, mtls, tunnelMode, pinnedServerSpki,
    });
    ...
    // die() routes through b.redact so a server-echoed secret-shaped substring
    // in err.message is masked, matching the redaction barrier every other
    // fatal exit uses (a bare console.error here would bypass it).
    die(`Init failed: ${err.message}`);
  }
}
...
async function cmdInitFromEnv() {
  try {
    ...
    const ed = await exchangeEnrollmentCode(server, code);
    // Guard the credential before saveSyncConfig -> keychain.store: a success
    // response with no api key must fail loudly, not store an empty key.
    if (!ed.apiKey || typeof ed.apiKey !== 'string' || !ed.apiKey.trim()) {
      throw new Error('Enrollment succeeded but the server returned no API key. If this is a certificate-reissue code, run "hermitstash-sync repair" instead of the init/enroll flow.');
    }
    ...
    let tunnelMode, pinnedServerSpki;
    if (_wantTunnel()) {
      pinnedServerSpki = await resolveTunnelPins(server, { interactive: false });
      tunnelMode = true;
    }
    ...
    // die() applies the b.redact barrier — see the interactive init path.
    die(`[init] Failed: ${err.message}`);
  }
}
...
async function cmdStart(args) {
  ...
  // Parent-handoff exemption: in `start --daemon` the parent has already
  // written this child's own PID into the pidfile (b.daemon.start does the
  // atomic write) before the child re-runs this guard — so seeing OUR OWN pid
  // is the handoff, not a second instance. Without this the detached child
  // dies on its own pidfile while the parent prints success, and `start
  // --daemon` never yields a running daemon.
  if (running === process.pid) running = false;
  if (running && process.env.HERMITSTASH_SELF_REPLACE === '1') {
    const handoffDeadline = performance.now() + b.constants.TIME.seconds(10);
    while (running && performance.now() < handoffDeadline) {
  ...
  // Before the rollback check below, which makes an irreversible decision from
  // the wall clock and must not act on an unvalidated one.
  //
  // Two paths depend on the clock being roughly right. The update probation
  // window false-clears on a forward jump and never ends on a backward one; the
  // certificate renewal threshold renews needlessly on a clock weeks behind and
  // fails to renew an expiring certificate on one weeks ahead. A laptop resumed
  // from sleep, a container with no real-time clock, or a dead time daemon can
  // all drift far enough to corrupt those silently.
  //
  // Unreachable time servers are not fatal, so an air-gapped install still
  // boots. HERMITSTASH_NTP_DISABLE skips the check; HERMITSTASH_NTP_STRICT
  // makes unreachable fatal. It runs once, not as a monitor.
  await _ntpBootGate();
  ...
  // Runs after the NTP gate so its Date.now()-based probation/rollback
  // decision is made against a clock the gate has already validated.
  ...
  daemon.installSignalHandlers(
    async () => {
      ...
      // here. No-op unless a self-update is on probation. Stamped BEFORE the
      // engine drain: receipt of the signal is already the clean-stop evidence,
      // and stamping after engine.stop() meant a drain that hit its phase
      // budget skipped the stamp and the next boot misread the stop as a
      // crash (spurious rollback).
  ...
  // Shared between the initial start below and the failed-update-install
  // recovery path, which must re-arm it after restarting the engine.
  const armWatchdog = () => {
    const t = setInterval(() => {
    ...
    t.unref();                                                                   // don't keep the event loop alive on shutdown
    return t;
  };
  ...
  if (cfg.autoUpdate !== false && !noAutoUpdate && !envDisable) {
    ...
    updaterInstance.start(async (install) => {
      ...
      try {
        ...
        // "Running version" not "current version": a failure after the swap
        // (marker write, successor spawn) leaves the NEW binary on disk while
        // this process still runs the old one — describe the process, not the
        // disk, so the message can't contradict the on-disk state.
        log.error('Auto-update install failed: ' + err.message + '. Resuming sync on the running version.');
        // The engine was stopped above for the swap. Without a restart the
        // daemon idles with syncing dead — and once the (unref'd) timers are
        // all that remain the event loop drains and the process silently
        // exits 0, which on the bare-binary shape (the only one with
        // self-replace enabled) means sync stops until an operator notices.
        // Re-read patterns from disk so a SIGHUP live-reload applied since
        // boot isn't reverted by this closure's stale boot-time arrays.
        try {
          const freshCfg = config.load();
          await engine.start(config.getIgnorePatterns(freshCfg), config.getIncludePatterns(freshCfg));
          watchdogTimer = armWatchdog();
          log.info('Sync engine restarted after failed update install');
        } catch (restartErr) {
          // Mirror the initial-start failure path: a daemon that can neither
          // update nor sync must exit loudly, not linger half-dead.
          log.error('Failed to restart sync engine after failed update install', restartErr);
          daemon.removePid();
          await _drainLogsBeforeExit();
          process.exit(1);
        }
...
function cmdStatus() {
  // Read-only probe: `status` is a Docker HEALTHCHECK / k8s-liveness / systemd
  // poll, so it must not reap a stale pidfile as a side effect (which would
  // desync a following `stop`). daemon.status() never unlinks.
  const running = daemon.status();
...
function cmdStats(args = []) {
  ...
  const running = daemon.status(); // read-only probe (see cmdStatus) — must not reap a stale pidfile
...
async function cmdStop() {
  // Windows now stops cooperatively: daemon.stop() writes the <PID_FILE>.stop
  // sentinel and the daemon's receiver (installSignalHandlers) runs the SAME
  // graceful teardown as POSIX — including updater.markGracefulShutdown(), which
  // the shutdown path stamps as its FIRST action (before the engine drain) so an
  // operator stop during the auto-update probation window is recorded as a clean
  // stop and never misread as a crash on the next boot. So there is no longer a
  // CLI-side stamp here: the daemon stamps for itself on both platforms. Only a
  // daemon whose event loop is wedged (never observes the sentinel within the
  // stop timeout) is escalated to TerminateProcess — and that case SHOULD roll
  // back on the next boot, exactly as a SIGKILL-escalated POSIX daemon does, so
  // stamping it would wrongly mask a hung-daemon rollback. daemon.stop() prints
  // the escalation note itself when that happens.
  await daemon.stop();
}
...
function cmdLog(args) {
  ...
  if (follow) {
    ...
    const repeater = b.safeAsync.repeating(() => {
      ...
      // A shrink means the log rotated (<prefix>.log → .gz, fresh .log) or was
      // truncated under us — re-follow from the start of the new file rather
      // than reading past its EOF (which would stall and lose content).
      ...
      if (stat.size > pos) {
        ...
        try {
          ...
          // Advance by the bytes ACTUALLY read and write only those: a short
          // read (a rotation landing between the stat and the read) must not
          // emit the zero-filled tail of buf as NUL bytes, nor skip content by
          // jumping pos to a stale stat.size past the new file's end.
          const bytesRead = nodeFs.readSync(fd, buf, 0, buf.length, pos);
          if (bytesRead > 0) {
...
async function cmdResync() {
  ...
  if (running) {
    ...
    if (process.platform === 'win32') {
      ...
      process.exitCode = 1; // the requested resync was NOT delivered — fail the exit code
      return;
    }
    ...
    try {
      ...
      process.exitCode = 1; // signal delivery failed — surface it via the exit code
    }
    return;
  }
  ...
  const cfg = loadConfigOrDie();
  await log.init({ level: cfg.logLevel });
...
Commands:
  ...
  init --tunnel     Setup for reaching the server inside a Tailscale/WireGuard
                    tunnel: pin the server's TLS leaf SPKI, then skip the
                    hostname/SAN check on connect (the tunnel + pin + mTLS bind
                    identity). Supply the pin out-of-band via
                    HERMITSTASH_PINNED_SERVER_SPKI, or confirm the one observed
                    at setup. Headless (HERMITSTASH_TUNNEL_MODE=1) requires the
                    supplied pin — it cannot confirm an observed one.
...
module.exports = { run, buildEnrollmentAgent, exchangeEnrollmentCode, safeServerLabel, resolveTunnelPins };
