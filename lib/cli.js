 ...
 * (trust-on-first-use), and that capture is then acknowledged EXPLICITLY. The
 * reason is specific to tunnel mode: an attacker on the network path at exactly
 * this moment can answer the probe with a certificate they own from any public
 * CA, and it is their SPKI that gets pinned. Because tunnel mode also skips the
 ...
 * with the hostname check on. Requiring an acknowledgement keeps that a
 * deliberate, visible act: interactive setup shows the pin and prompts,
 * headless setup requires HERMITSTASH_TUNNEL_PIN_ACK.
 */
async function resolveTunnelPins(server, opts) {
  ...
  console.log(interactive ? '\nCapturing the server SPKI pin for tunnel mode...'
    : '[init] Capturing the server SPKI pin for tunnel mode...');
  const pin = await captureTunnelPin(server);
  console.log('  Server SPKI pin: ' + pin);

  const ackEnv = b.safeEnv.readVar('HERMITSTASH_TUNNEL_PIN_ACK', { type: 'string', default: '' }).trim();
  if (/^(1|true|yes|on)$/i.test(ackEnv)) return [pin];

  if (!interactive) {
    throw new Error(
      'tunnel mode captured the server SPKI pin ' + pin + ' but has no confirmation. Because tunnel ' +
      'mode does not check the hostname, this pin is what binds the server\'s identity — confirm it ' +
      'matches the server before trusting it. Either set HERMITSTASH_PINNED_SERVER_SPKI to the pin ' +
      'you obtained from the server out-of-band (preferred), or set HERMITSTASH_TUNNEL_PIN_ACK=1 to ' +
      'accept the pin observed just now.');
  }
...
Commands:
  ...
  init --tunnel     Setup for reaching the server inside a Tailscale/WireGuard
                    ...
                    at setup. Headless: HERMITSTASH_TUNNEL_MODE=1 (plus
                    HERMITSTASH_TUNNEL_PIN_ACK=1 to accept an observed pin).
