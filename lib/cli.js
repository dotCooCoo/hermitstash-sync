...
async function cmdInit(args = []) {
  ...
  try {
    ...
    if (_wantTunnel(args)) {
      ...
      pinnedServerSpki = [await captureTunnelPin(server)];
      tunnelMode = true;
      console.log('  Pinned server SPKI: ' + pinnedServerSpki[0]);
    }
...
async function cmdInitFromEnv() {
  try {
    ...
    let tunnelMode, pinnedServerSpki;
    if (_wantTunnel()) {
      console.log('[init] Capturing the server SPKI pin for tunnel mode...');
      pinnedServerSpki = [await captureTunnelPin(server)];
      tunnelMode = true;
      console.log('[init] Pinned server SPKI: ' + pinnedServerSpki[0]);
    }
...
Commands:
  ...
  init --tunnel     Setup for reaching the server inside a Tailscale/WireGuard
                    tunnel: capture + pin the server's TLS leaf SPKI now, then
                    skip the hostname/SAN check on connect (the tunnel + pin +
                    mTLS bind identity). Headless: set HERMITSTASH_TUNNEL_MODE=1.
