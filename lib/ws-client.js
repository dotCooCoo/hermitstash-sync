...
// TLS options that decide whether the server is verified at all. `config.tls`
// is an UNDECLARED passthrough — it is absent from the config DEFAULTS and
// config.validate() has no rule for it, so config.load()'s `{ ...DEFAULTS,
// ...parsed }` carries any such key straight from config.json into the dial.
// Spread last (as it was), a config carrying
// `"tls": {"rejectUnauthorized": false}` disables chain validation on this
// transport — and because Node only invokes `checkServerIdentity` when the
// chain verified, it silently voids the SPKI pin along with it. That turns
// write access to config.json into full interception of the control channel,
// including the Bearer API key, which on macOS/Windows otherwise lives in the
// OS keychain rather than the config dir. Strip the verification-critical keys
// unless the operator sets the explicit, documented escape hatch — the same
// posture as HERMITSTASH_ALLOW_CLASSICAL_TLS / HERMITSTASH_ALLOW_PLAINTEXT.
function _allowInsecureTls() {
  const raw = b.safeEnv.readVar('HERMITSTASH_ALLOW_INSECURE_TLS', { type: 'string', default: '' }).trim();
  return /^(1|true|yes|on)$/i.test(raw);
}

// Ignore the WHOLE bag by default rather than filtering known-dangerous keys.
// A denylist is the wrong shape here: nearly every tls.connect option can defeat
// server identity, and this object is spread LAST, so anything left un-stripped
// overrides what we just set. `ca` alone REPLACES Node's trust store (see
// _buildCaBundle in http-client.js) so an inline attacker root turns a
// chain-valid handshake into a full intercept; `servername` retargets the very
// hostname the identity check runs against; `minVersion`/`maxVersion`/`ciphers`
// drop the connection to TLS 1.2, which the post-handshake post-quantum floor
// reads as "at floor" because a static-RSA handshake reports no ephemeral group.
// Nothing in the client sets config.tls — only the test harness does — so the
// safe default is to honor none of it and require the documented escape hatch.
...
// Stable id for this installation, read once per process and cached.
//
// Sent on every WebSocket upgrade so the server can drop the connection this
// same client left behind, instead of counting a not-yet-reaped socket against
// the per-key connection ceiling and refusing the reconnect. It is persisted so
// a restarted daemon still supersedes its previous run's connections; a
// per-process id would leave exactly those ghosts stranded.
//
// It identifies, it does not authenticate — the API key and client certificate
// do that, and the server only ever uses this value to choose among connections
// that already belong to the same key. If it cannot be read or written (a
// read-only config dir, a container with no persistent volume) the client simply
// sends no header and the server behaves as it always did.
let _instanceId;
function instanceId() {
...
// would override.
//
// `ecdhCurve` only. Node has no `groups` TLS option — configSecureContext never
// reads one — so the `groups` key that used to sit here was accepted and
// discarded. It read as insurance against Node honouring one key or the other,
// but only `ecdhCurve` was ever consulted, and only `ecdhCurve` is validated.
