...
var TLS_VERIFY_KEYS = ['rejectUnauthorized', 'checkServerIdentity'];
...
function _sanitizedConfigTls(raw) {
  if (!raw || typeof raw !== 'object') return {};
  if (_allowInsecureTls()) return raw;
  const out = {};
  const stripped = [];
  for (const k of Object.keys(raw)) {
    if (TLS_VERIFY_KEYS.indexOf(k) !== -1) { stripped.push(k); continue; }
    out[k] = raw[k];
  }
  if (stripped.length) {
    log.warn('Ignoring TLS verification override(s) from config (' + stripped.join(', ') +
      ') — server verification stays on. Set HERMITSTASH_ALLOW_INSECURE_TLS=1 to permit them deliberately.');
  }
  return out;
}
...
class WsClient extends EventEmitter {
  ...
  _dial() {
    ...
    const tlsOpts = isHttps ? _pqcTlsOpts({
      ...
      // config.tls is an operator/test passthrough; _sanitizedConfigTls drops the
      // verification-critical keys so it can never disable chain validation or
      // displace the SPKI-pin check above (see TLS_VERIFY_KEYS). A harness that
      // genuinely needs a self-signed peer sets HERMITSTASH_ALLOW_INSECURE_TLS=1.
      ..._sanitizedConfigTls(this._config.tls),
    }) : null;
