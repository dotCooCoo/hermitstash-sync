...
const bWsClient  = require('../vendor/blamejs/lib/ws-client');
...
class WsClient extends EventEmitter {
  ...
  close() {
    ...
    if (this._b) {
      try { this._b.close(1000, 'client-close'); } catch (_e) {}
  ...
  _dial() {
    ...
    const sni = net.isIP(serverUrl.hostname) ? undefined : serverUrl.hostname;
    ...
    let client;
    try {
      client = bWsClient.connect(wsUrl, {
        headers: {
    ...
    client.on('message', (data, isBinary) => {
      ...
      let msg;
      try {
        msg = bSafeJson.parse(data, { maxBytes: MAX_MESSAGE_BYTES });
      } catch (err) {
        log.warn('Invalid JSON from server', { err: err.message, head: String(data).slice(0, 200) });
        return;
      }
