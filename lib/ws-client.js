...
class WsClient extends EventEmitter {
  ...
  _isTerminalWsError(err) {
    ...
    if (err.code === 'ws-client/bad-status') {
      const status = _wsErrorStatus(err);
      // Terminal for every non-5xx handshake status, mirroring blamejs's own
      // _wsErrorIsPermanent classifier. Only a 5xx is transient (a momentarily
      // overloaded server the wrapper self-heals against). A 1xx/2xx/3xx (e.g. a
      // misrouted reverse proxy answering the Upgrade with 200 or a 302 redirect
      // instead of 101) is a permanent misconfiguration: without this the
      // wrapper classified it neither terminal nor transient-with-signal and
      // reconnected forever with no upgrade_rejected and no operator-visible
      // ERROR. An unparseable status (0) also fails closed here rather than
      // looping silently.
