...
const { TLS_GROUPS, TLS_MIN_VERSION, RECONNECT_DELAYS, HEARTBEAT_TIMEOUT_MS } = require('./constants');
  ...
  _doConnect() {
    ...
    if (isHttps) {
      ...
      if (this._config.mtls) {
        if (this._config.mtls.cert) options.cert = fs.readFileSync(this._config.mtls.cert);
        if (this._config.mtls.key) options.key = fs.readFileSync(this._config.mtls.key);
        if (this._config.mtls.ca) options.ca = fs.readFileSync(this._config.mtls.ca);
      }
    }
