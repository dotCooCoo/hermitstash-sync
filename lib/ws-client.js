...
const { VERSION, RECONNECT_DELAYS, HEARTBEAT_TIMEOUT_MS } = require('./constants');
const { buildTlsOptions } = require('./http-client');
...
class WsClient extends EventEmitter {
  ...
  /**
   * Re-read mTLS cert/key/CA files from disk and refresh the cached buffers.
   * Called by sync-engine after a CA rotation (ca:rotation message) has
   * persisted new credentials. Without this, the next reconnect would re-use
   * the stale buffers cached at construction time and fail against the new CA.
   */
  reloadMtlsCerts() {
    if (!this._config.mtls) return;
    try {
      if (this._config.mtls.cert) this._cachedCert = fs.readFileSync(this._config.mtls.cert);
      if (this._config.mtls.key)  this._cachedKey  = fs.readFileSync(this._config.mtls.key);
      if (this._config.mtls.ca)   this._cachedCa   = fs.readFileSync(this._config.mtls.ca);
  ...
  _doConnect() {
    ...
    if (isHttps) {
      // Pass already-loaded buffers (cached on first connect) as the mtls
      // source so buildTlsOptions doesn't re-read them from disk on every
      // reconnect — the helper's `load` branch recognises Buffers and
      // passes them through.
      Object.assign(options, buildTlsOptions(
        { cert: this._cachedCert, key: this._cachedKey, ca: this._cachedCa },
        this._config.tls, // e.g. rejectUnauthorized for self-signed test CAs
      ));
    }
    ...
    req.on('upgrade', (res, socket, head) => {
      ...
      // Server uses this GUID (hermitstash-private/lib/ws.js) — must match.
      const expectedAccept = crypto
        .createHash('sha1')
        .update(wsKey + '258EAFA5-E914-47DA-95CA-5AB5DC11CE46')
        .digest('base64');
    ...
    req.on('response', res => {
      ...
      res.on('end', () => {
        ...
        this.emit('upgrade_rejected', { status: res.statusCode, body });
