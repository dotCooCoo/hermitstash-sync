...
class SyncEngine extends EventEmitter {
  ...
  // is a FIFO async lock from the vendored framework — zero npm deps.
  ...
  _validateMtlsTrio(certPem, keyPem, caPem) {
    ...
    if (!leaf.verify(caCert.publicKey)) {
      throw new Error('rotated leaf does not chain to rotated CA');
    }
  }
  ...
  _recoverMtlsFromBackup() {
    ...
    try {
      if (nodeFs.existsSync(longPath(cert)) && nodeFs.existsSync(longPath(key))) {
        const certPem = nodeFs.readFileSync(longPath(cert), 'utf8');
        const keyPem = nodeFs.readFileSync(longPath(key), 'utf8');
      ...
      // Unparseable live cert/key — treat as torn so the backup is tried.
      torn = true;
    }
  ...
  async _checkCertExpiry() {
    ...
    try {
      ...
      var endDate;
      try {
        var x509 = new nodeCrypto.X509Certificate(nodeFs.readFileSync(longPath(certPath)));
  ...
  async _reconcileCaseRename(existing, relativePath, fullPath, fileId, checksum, size, seq) {
    ...
    const oldFullPath = this._safePath(oldKey);
    try {
      if (oldFullPath && oldFullPath !== fullPath && nodeFs.existsSync(longPath(oldFullPath))) {
        ...
        const tmp = fullPath + '.hs-case.' + nodeCrypto.randomBytes(4).toString('hex'); // allow:raw-randombytes-token — local temp suffix for the two-step case rename, not a security token
  ...
  async _handleFileRenamed(msg) {
    ...
    try {
      ...
      // A SYNCED row is only honest when the bytes actually landed at the new
      // path AND match the server's content. If the source file was absent on
      // disk (an earlier download left an ERROR/PENDING_DOWNLOAD row, say), the
      // move above was skipped and newFullPath holds no bytes — writing SYNCED
      // there strands a phantom row the reconcile sweep never re-downloads (it
      // only re-drives PENDING_DOWNLOAD/ERROR, and the local rescan only walks
      // files that exist), permanently losing the file locally while reporting
      // it synced. A rename event may omit checksum (pure rename, content
      // unchanged) — the moved bytes keep their known checksum; a checksum that
      // differs from our last-synced value means the rename also changed
      // content. In every not-SYNCED case fall through to the delete-old +
      // re-download-new recovery below, which upserts PENDING_DOWNLOAD and owns
      // the seq advance only once the bytes land.
      const landed = nodeFs.existsSync(longPath(newFullPath));
  ...
  async _executeDeleteLocked(relativePath, existing) {
    ...
    if (this._state === SYNC_STATE.DISCONNECTED || this._resyncing || !this._http) return false;
  ...
  async _handleLocalRename(oldPath, newPath, pending, checksum, size, mtime) {
    ...
    try {
      ...
      const deleted = await this._executeDelete(oldPath, pending.existing);
      if (!deleted) return;
  ...
  _detectCaseFolding() {
    let probe = null;
    try {
      const token = nodeCrypto.randomBytes(6).toString('hex'); // allow:raw-randombytes-token — local FS-probe filename, not a security token
      const lower = nodePath.join(this._config.syncFolder, '.hs-case-probe-' + token);
      const upper = nodePath.join(this._config.syncFolder, '.HS-CASE-PROBE-' + token);
