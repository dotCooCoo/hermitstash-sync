...
// Ceiling for the pre-commit CA-rotation capability probe (a TLS 1.3 mutual
// loopback handshake with the incoming identity). A stuck probe must not wedge
// the rotation handler, so the loopback resolves `false` after this and the
// rotation is declined rather than hung. Matches the server-side migration
// gate's probe ceiling (hermitstash-private lib/mtls-migrate.js).
const MTLS_PROBE_TIMEOUT_MS = C.TIME.seconds(3);
...
// macOS and any other NFD-surfacing volume return decomposed forms from
// readdir, while the server stores the composed form the file was uploaded
// under. Left alone they are distinct keys, and the same file churns as a
// phantom remove and add on every reconnect. Every relativePath is normalised
// at each boundary — inbound handlers, the state database key, the watcher —
// so one byte-stable identity flows through the whole pipeline.
...
class SyncEngine extends EventEmitter {
  constructor(config, apiKey) {
    ...
    // A local upload and a same-path download have to run in strict order. The
    // pre-await suppression check is a TOCTOU window on its own: a local edit
    // passes it, awaits its hash, then uploads while a download for the same
    // path is in flight. Holding a per-path mutex across the hash, upsert and
    // transfer closes it, while unrelated paths still run in parallel. An idle
    // slot is dropped so the map cannot grow unbounded.
    ...
    // The seq of the trailing catch-up-complete heartbeat when it is still
    // ABOVE our applied cursor (i.e. we can't satisfy it yet). Lets the
    // reconcile sweep tell a genuinely wedged cursor (a top-of-batch transfer
    // failed, so lastSeq stayed below the tip and no live event will re-advance
    // it) from a fresh catch-up that simply hasn't delivered its heartbeat. 0
    // = no unsatisfiable tip seen this session.
    this._catchUpTipSeq = 0;
    // Lowest catch-up seq a handler failed to apply this session (a fail-closed
    // delete on a locked/unhashable file advances nothing and leaves no
    // PENDING_DOWNLOAD/ERROR row). Gates the catch-up wedge redial so it can't
    // replay past this seq and silently leapfrog the un-applied event. 0 = none.
    this._lowestUnappliedSeq = 0;
    ...
    // Paths whose last transfer failed PERMANENTLY (over the effective per-file
    // size cap, or a server 4xx that won't change on retry). The reconcile sweep
    // skips these so a doomed transfer isn't re-streamed every ~5-minute tick;
    // a path is removed on any successful transfer, and the whole set is cleared
    // on resync (a config change — e.g. a raised maxFileSize — arrives that way).
    ...
    // Single-flight guard for the periodic reconcile timer: true from the moment
    // a timer-driven reconcile is chained onto the apply chain until it settles,
    // so an overlapping timer firing (or a slow reconcile that outruns the
    // interval) drops instead of piling redundant reconciles onto the chain.
    ...
    // The socket pool is the hard wall; this keeps the promise queue in front
    // of it bounded, so copying ten thousand files in does not materialise ten
    // thousand in-flight promises waiting for a socket.
    //
    // Math.trunc(Number(...)) rather than `| 0`, because ToInt32 wraps a value
    // at or above 2^31 to a negative number, which the clamp then reads as 1 —
    // a silent throttle to single-file uploads where 16 was asked for.
    const cc = Math.max(1, Math.min(16, Math.trunc(Number(config.uploadConcurrency)) || UPLOAD_DEFAULT_CONCURRENCY)); // allow:raw-byte-literal — concurrency clamp upper bound (count, not bytes)
    this._uploadPool = b.promisePool.create({ concurrency: cc });
    this._registerPoolStatsProvider();
  }

  // stop() clears the provider slot, so the metrics module releases this engine
  // rather than sampling a drained pool. Called from the constructor AND from
  // start(), because a stop-then-start on the same instance — which the
  // failed-install recovery does — would otherwise leave pool telemetry frozen
  // at its pre-stop values for the rest of the daemon's life.
  _registerPoolStatsProvider() {
  ...
  // Every _suppressPath needs exactly one _releasePath on every exit path; the
  // count is what survives overlapping operations on one path. The key is
  // case-folded on a case-folding host, or a "Foo.txt" download would not
  // suppress a "foo.txt" watcher event for the same inode and the file would
  // re-upload mid-download.
  _suppKey(p) {
  ...
  // Each of the three writes is atomic on its own. The SET is not: a crash
  // partway through can land a new certificate and key beside the old CA, or
  // any other torn combination. The rotation handler compensates with staging,
  // chain validation, a backup and a boot-time recovery from it — this helper
  // only promises that no individual file is left half-written.
  ...
  _validateMtlsTrio(certPem, keyPem, caPem) {
    ...
    // The CA must issue the leaf AND assert basicConstraints cA:TRUE. Node's
    // own verify() and checkIssued() cover the signature and the DN linkage
    // but not the cA bit, so an end-entity certificate whose key happened to
    // sign the leaf would be accepted as a trust anchor — the basicConstraints
    // bypass. This test checks the cA bit first and never throws.
  ...
  // Proves this runtime can complete a real mutual handshake with a candidate
  // identity — which parsing, key-matching and chain-checking do not cover.
  // OpenSSL can chain-verify a certificate whose signature algorithm it cannot
  // drive through the TLS record layer, and the handshake then fails on the
  // CertificateVerify.
  //
  // The loopback puts the same leaf on both ends. With requestCert on, the
  // client signs its own CertificateVerify and verifies the peer's before
  // secureConnect fires, so one callback proves both directions. Identity
  // checking is off so that an EKU or SAN mismatch on the reused leaf cannot
  // masquerade as a capability gap — only a real algorithm failure stops the
  // connection. Resolves false on any error, so an unusable algorithm is
  // declined rather than committed.
  //
  // codeql[js/disabling-certificate-validation] intentional: a throwaway
  // in-process loopback carrying no application data, existing only to test
  // whether this runtime can drive the algorithm through TLS. The daemon's
  // real connections keep full verification and SPKI pinning.
  _probeMtlsTlsCapability(certPem, keyPem, caPem) {
    return new Promise((resolve) => {
      ...
      const done = (result) => {
        if (settled) return;
        settled = true;
        try { if (timer) clearTimeout(timer); } catch (_e) { /* noop */ }
        try { if (client) client.destroy(); } catch (_e) { /* noop */ }
        try { if (server) server.close(); } catch (_e) { /* noop */ }
        resolve(result);
      };
      const timer = setTimeout(() => done(false), MTLS_PROBE_TIMEOUT_MS);
      ...
      try {
        server = nodeTls.createServer({
          ...
          ca: [caPem],
          requestCert: true,
          rejectUnauthorized: false, // allow:tls-verify-disabled — server side of the in-process 127.0.0.1 loopback capability probe; carries no data
          minVersion: 'TLSv1.3',
        }, (socket) => {
          socket.on('error', () => { /* peer teardown during probe — ignored */ });
          socket.end();
        });
        ...
        server.listen(0, '127.0.0.1', () => {
          ...
          client = nodeTls.connect({
            ...
            ca: [caPem],
            minVersion: 'TLSv1.3',
            rejectUnauthorized: false, // allow:tls-verify-disabled — client side of the loopback probe; isolates algorithm capability from chain/EKU authorization
            checkServerIdentity: () => undefined, // allow:tls-verify-disabled — loopback reuses the client leaf as the server; SAN/hostname is irrelevant to a capability test
          }, () => {
            // secureConnect fired without a TLS-layer error → the runtime
            // signed + verified the new algorithm's CertificateVerify.
            done(true);
          });
          client.on('error', () => done(false));
        });
      } catch (_e) {
        // Synchronous secure-context failure (e.g. a key that doesn't match
        // the cert fails SSL_CTX_check_private_key at construction).
        done(false);
      }
    });
  }

  // Best-effort human label for a candidate CA's signature algorithm, used
  // only in the capability-decline log line. Prefers the server-sent
  // `caAlgorithm` hint (authoritative), then the CA public key's type as node
  // reports it, then a generic phrase — the label is diagnostic text, never a
  // control-flow input.
  _deriveCaAlgorithmLabel(caPem, hint) {
    if (hint && typeof hint === 'string') return hint;
    try {
      const ca = new nodeCrypto.X509Certificate(caPem);
      const t = ca.publicKey && ca.publicKey.asymmetricKeyType;
      if (t) return String(t);
    } catch (_e) { /* fall through to generic label */ }
    return 'the new signature algorithm';
  }

  // Remove any `.next` staging siblings for the mTLS trio. Called on a declined
  // rotation (so a stale stage from a prior aborted attempt can't be read as a
  // torn-swap signal by boot recovery) and immediately before a fresh stage.
  // Best-effort: a missing file is fine (`force: true`), any other error is
  // swallowed rather than aborting the rotation over cleanup.
  _cleanupRotationStaging(mtls) {
    if (!mtls) return;
    for (const p of [mtls.cert, mtls.key, mtls.ca]) {
      if (!p) continue;
      try { nodeFs.rmSync(longPath(p + '.next'), { force: true }); } catch (_e) { /* noop */ }
    }
  }

  // Restores the identity from the pre-rotation backup when the live trio is
  // torn — a rotation that crashed mid-swap — or a stage file was left behind.
  // Runs at boot, before the HTTP client caches anything. Best-effort: a
  // missing or partial backup leaves the live files alone and says so.
  ...
  async start(ignorePatterns, includePatterns) {
    ...
    this._stopped = false;
    // Re-register the upload-pool stats provider: stop() cleared it, and this
    // engine instance may be reused (auto-update failed-install recovery does
    // stop()+start() on the same object). Idempotent (single last-writer slot).
    this._registerPoolStatsProvider();
    ...
    // state so it recovers regardless of where lastSeq landed. Routed through
    // the apply chain (NOT a bare _recoverPending) so the sweep serializes
    // against live catch-up events for the same path — the reconcile's
    // conflict-copy + re-download must not interleave with a live handler
    // mutating the same row — and single-flighted so overlapping firings can't
    // pile the chain up with redundant reconciles.
    this._reconcileTimer = setInterval(() => {
      this._enqueueReconcile();
    ...
    this._watcher.on('rescan', () => this._enqueueLocalRescan('Watcher recovered — re-scanning synced folder for changes missed while detection was off'));
    ...
    this._ws.on('open', () => {
      // A local change that fired while the engine was in ERROR is dropped by
      // _onLocalChange, and nothing else re-detects it — the reconnect path has
      // no folder reconcile of its own — so the rescan below recovers it.
      //
      // Changes during RECONNECTING are NOT lost: uploads and deletes travel
      // over HTTP, which stays live. The rescan is belt-and-braces there. Do
      // not remove it on the premise that those changes were dropped.
      ...
      // Cleared for this session. A prior session that received its caught-up
      // heartbeat and then dropped before the queue drained would otherwise
      // leave the latch set, and this session would report SYNCED the instant
      // the pending count touched zero, with catch-up events still queued.
      this._caughtUpAtSeq = false;
      // Clear the unsatisfiable-tip marker for the fresh session so the wedge
      // recovery below can't fire off a stale value from a prior connection.
      this._catchUpTipSeq = 0;
      this._lowestUnappliedSeq = 0; // fresh session — forget prior fail-closed events
      ...
      if (wasDownControlChannel && !(lastSeq === 0 && this._config.shareId)) {
        this._enqueueLocalRescan('Control channel reconnected — re-scanning synced folder for local changes missed while disconnected');
      }
    });
  ...
  async stop() {
    ...
    // Await in-flight downloads before destroying the HTTP client and closing
    // the state DB, mirroring resync(). Without this, destroy() aborts a
    // download mid-stream and its catch calls updateFileStatus against a
    // closed DB — a swallowed 'apply failed' error that also skips the ERROR
    // persistence the next boot's recovery keys off. The post-await _stopped
    // guard still prevents any SYNCED write, so no partial download is
    // accepted; this just lets each in-flight download settle cleanly.
    if (this._activeDownloadPromises && this._activeDownloadPromises.size > 0) {
      try { await Promise.allSettled([...this._activeDownloadPromises]); }
      catch { /* allow:silent-catch — allSettled never rejects; guard is belt-and-braces */ }
    }
  ...
  resync() {
    // Never start a resync once the engine is stopping. A SIGHUP delivered
    // inside the graceful-stop window (e.g. logrotate's postrotate firing
    // while the supervisor sends SIGTERM) would otherwise run clearAll() and
    // re-open the WebSocket against state the teardown is draining/closing —
    // wiping local sync state and spawning a doomed reconnect moments before
    // exit. Matches the _stopped guard every other post-await writer honors.
    if (this._stopped) {
      log.debug('Ignoring resync — engine is stopping');
  ...
  async _resyncOnce() {
    ...
    // A resync re-pulls everything from the server (with any new config, e.g. a
    // raised maxFileSize), so a prior permanent-failure mark must not survive it.
  ...
  // Each message joins the apply chain, so handlers run one at a time in seq
  // order. A throw is caught and its seq recorded as un-applied, which caps the
  // cursor one below the gap — otherwise a later event that succeeded would
  // advance the cursor past the failed one, the server would never re-deliver
  // it, and that change would be silently lost. With the cap in place a
  // reconnect asks from below the gap and the server replays it.
  _onServerMessage(msg) {
    if (this._stopped) return;
    this._applyChain = this._applyChain
      .then(() => this._applyServerMessage(msg))
      .catch((err) => {
        // Cap the cursor below the failed event so a reconnect re-delivers it,
        // exactly as the explicit fail-closed branches (_handleFileRemoved /
        // _reconcileCaseRename) already do.
        this._noteUnappliedSeq(msg && msg.seq);
        log.error('Server event apply failed — will retry on next catch-up', {
  ...
  /**
   * The server has regenerated the CA and sent new credentials.
   *
   * The three files are not one atomic unit on disk, so the candidate trio is
   * staged and fully validated before the live identity is touched — a failure
   * at that point leaves the working identity alone and the server times its
   * own rotation out. Only then is the current trio backed up, the stage
   * swapped in, the caches reloaded, and the acknowledgement sent. A crash
   * anywhere in between is repaired from that backup at boot.
   */
  async _handleCaRotation(msg) {
    const { newCaPem, newCertPem, newKeyPem, restartInMs, dryRun, caAlgorithm } = msg;
    ...
    // 2. VALIDATE first — reject a structurally bad trio before any live file
    // is touched. This proves the trio parses, the key matches the cert, and
    // the leaf chains to a cA:TRUE issuer — but NOT that this client's TLS
    // runtime can actually complete a handshake with the new signature
    // algorithm (parsing + chain-verify are wired into OpenSSL earlier than
    // the TLS record layer for a given algorithm). A validation failure is a
    // SECURITY reject — the wrong CA or a tampered trio.
    try {
      ...
      this._cleanupRotationStaging(mtls);
      return;
    }

    // Before committing, prove this runtime can actually handshake with the
    // incoming identity. When the server moves the CA to an algorithm newer
    // than this client's bundled TLS stack, a structurally valid trio would be
    // committed and then fail every reconnect forever — and boot recovery
    // cannot help, because it only repairs a TORN swap, not a clean swap of an
    // unusable certificate. Declining here leaves the working identity in place.
    //
    // Not a security rejection, and deliberately not acknowledged: the server
    // times this client out and retries once its runtime can handle the
    // algorithm.
    const algoLabel = this._deriveCaAlgorithmLabel(newCaPem, caAlgorithm);
    let capable = false;
    try {
      capable = await this._probeMtlsTlsCapability(newCertPem, newKeyPem, newCaPem);
    } catch (_err) {
      capable = false;
    }
    if (!capable) {
      log.error('CA rotation declined — this sync client\'s TLS runtime cannot complete a mutual handshake with the new CA\'s signature algorithm, so committing it would break every reconnect. Keeping the current identity untouched; upgrade this sync client to a build whose Node/OpenSSL supports the algorithm, then the server\'s next rotation will apply. No files were changed', { algorithm: algoLabel });
      this._cleanupRotationStaging(mtls);
      return;
    }

    // Clear any stale .next staging from a prior aborted attempt so the swap
    // below starts clean and boot recovery can't later read an orphan as a
    // torn-swap signal.
    this._cleanupRotationStaging(mtls);
  ...
  // Renames a locally-modified file aside before a download overwrites it, so
  // the user's bytes survive. Best-effort: a failure logs and lets the download
  // proceed, since refusing would block that one file forever.
  //
  // knownLocalChecksum names bytes the caller knows are already-synced content
  // it just relocated — a server rename that also changed content moves the
  // old, undiverged bytes to the new path. Those are not a local divergence,
  // and copying them aside would spawn a spurious conflict file on every
  // rename-and-edit.
  async _maybeSaveConflictCopy(relativePath, fullPath, incomingChecksum, knownLocalChecksum) {
    try {
      if (!nodeFs.existsSync(longPath(fullPath))) return;
      let actualLocal = null;
      try {
        actualLocal = await hashFile(fullPath);
      } catch (err) {
        // Could not hash the local file (oversize past the ceiling, a special
        // file, or unreadable). We CANNOT confirm it matches the incoming bytes,
        // so we must not let the download silently overwrite it. Fall through
        // with actualLocal=null so the protective rename below preserves it as a
        // conflict copy — fail closed rather than destroying possibly-diverged
        // bytes. (A lock-class failure makes the rename below fail too, which
        // the download's own locked-rename retry then surfaces.)
        log.warn('Could not hash local file before a remote overwrite — preserving it as a conflict copy', { relativePath, error: err.message });
      }
      // Skip ONLY when we positively know it is safe: the local bytes match the
      // incoming bytes, OR they are the caller's known last-synced bytes just
      // relocated (a rename that also changed content). A null (indeterminate)
      // hash falls through to the rename (fail closed).
      if (actualLocal && (actualLocal === incomingChecksum ||
          (knownLocalChecksum && actualLocal === knownLocalChecksum))) return;
  ...
  async _handleFileAdded(msg, knownLocalChecksum) {
    ...
    try {
      ...
      // knownLocalChecksum (rename-fallback caller only) suppresses a spurious
      // conflict copy when the on-disk bytes are the last-synced content the
      // rename just relocated. When there's no explicit rename value but the
      // path IS already tracked, fall back to its last-synced serverChecksum so
      // a device still holding the un-diverged prior version isn't given a
      // spurious conflict copy (same class as _handleFileReplaced).
      await this._maybeSaveConflictCopy(relativePath, fullPath, checksum, knownLocalChecksum || (existing && existing.serverChecksum));
  ...
  async _reconcileCaseRename(existing, relativePath, fullPath, fileId, checksum, size, seq) {
    ...
    try {
      if (oldFullPath && oldFullPath !== fullPath && nodeFs.existsSync(longPath(oldFullPath))) {
        ...
        try {
          ...
          // The first rename moved the user's bytes to the temp name but the
          // second failed — restore them to the original path so they are not
          // stranded under a temp name the boot sweep deletes (permanent loss
          // if the bytes were diverged). Best-effort; rethrow the original
          // failure so the catch below pins the cursor for a re-delivery.
          try { b.atomicFile.renameWithRetry(longPath(tmp), longPath(oldFullPath)); } catch { /* allow:silent-catch — best-effort restore; the temp orphan is still boot-sweep-collectable */ }
          throw secondErr;
        }
      }
    } catch (err) {
      ...
      // Pin the cursor below this failed event so the next reconnect re-delivers
      // it. Not advancing seq here is NOT sufficient on its own: a later
      // successful event's _updateSeq would advance lastSeq past this seq (no
      // watermark), and since=lastSeq would then tell the server never to
      // re-send it — the local file keeps its stale casing and any content
      // update the event also carried is permanently lost (the reconcile sweep
      // won't re-drive a row still marked SYNCED). Mirrors the fail-closed
      // _handleFileRemoved branch.
      this._noteUnappliedSeq(seq);
      return;
    }
    ...
    // The local bytes at the (now case-corrected) path diverge from the
    // incoming server content. Preserve them as a conflict copy before the
    // re-download overwrites them — the same guard the live _handleFileReplaced
    // path applies. Pass existing.serverChecksum so a device still holding the
    // un-diverged prior version isn't given a spurious conflict copy.
    await this._maybeSaveConflictCopy(relativePath, fullPath, checksum, existing.serverChecksum);
    const ok = await this._downloadFile(relativePath, fileId, checksum, size);
    if (ok) this._updateSeq(seq);
  }
  ...
  async _handleFileReplaced(msg) {
    ...
    try {
      ...
      // Passing the last-synced value matters on the ordinary multi-device
      // edit, where another device changed the file and this one still holds
      // the undiverged previous version: the disk bytes equal it, so no
      // conflict copy is made. Without that, every such edit would spawn a
      // conflict file of already-synced content, which the watcher would then
      // re-upload and fan out to every device. A genuine divergence — disk
      // matching neither value — is still preserved.
      await this._maybeSaveConflictCopy(relativePath, fullPath, checksum, existing && existing.serverChecksum);

      // Preserve the pre-replace last-synced marker (localChecksum). Spreading
      // `existing` keeps it so that if this download fails, the reconcile
      // sweep can tell the on-disk bytes are the un-diverged relocated content
      // (disk === localChecksum) and re-download without a spurious conflict
      // copy. A bare upsert would NULL localChecksum, and the sweep would then
      // conflict-copy already-synced bytes and fan them out — the multi-device
      // spurious-conflict class the live path guards against.
      stateDb.upsertFile({
        ...existing,
        relativePath,
        serverFileId: fileId,
  ...
  async _handleFileRemoved(msg) {
    ...
    try {
      const existing = this._lookupTracked(relativePath);
      // "Last synced" = what actually reached the server (serverChecksum), NOT
      // localChecksum. localChecksum is unsafe as a proxy: a PENDING_UPLOAD /
      // ERROR row's localChecksum is bytes that never landed on the server, and
      // an untracked file (existing===null) was never synced at all. Basing the
      // uncontested-delete decision on localChecksum would let a same-path
      // multi-device create+delete race destroy never-synced local data.
      const syncedChecksum = existing && existing.serverChecksum;
      let applied = true;
      ...
      if (nodeFs.existsSync(longPath(fullPath))) {
        ...
        // The delete is uncontested ONLY when the on-disk bytes still match
        // what we actually synced to the server. Anything else — locally
        // diverged bytes, OR a file that was never confirmed on the server
        // (no serverChecksum: a never-landed upload or an untracked file
        // surfaced by a same-path multi-device race) — is preserved as a
        // conflict copy rather than destroyed, matching _handleFileAdded's
        // fail-safe. An unhashable file fails closed (keep + retry).
        const uncontested = actualLocal && syncedChecksum && actualLocal === syncedChecksum;

        if (uncontested) {
          // Uncontested delete — bounded retry on a Windows/Dropbox lock.
          applied = await this._unlinkWithRetry(relativePath, fullPath);
        } else if (actualLocal) {
          // Diverged from what we synced, or never synced to the server at all
          // — keep the user's bytes as a conflict copy instead of honoring a
          // delete that would lose them.
          try {
            const conflict = b.atomicFile.conflictPath(fullPath);
            b.atomicFile.renameWithRetry(longPath(fullPath), longPath(conflict));
            metrics.record.conflict();
            ...
            applied = false;
            log.error('Could not preserve local file before remote delete — keeping the local file and the DB row; close any app holding the file and the next reconnect retries', { relativePath, error: err.message });
          }
        } else {
          // Indeterminate: the file is present but we could NOT hash it (it grew
          // past the size ceiling, became a special file, or is unreadable), so
          // we cannot confirm it is safe to delete. Fail CLOSED — keep the local
          // file and the DB row and do NOT advance seq, rather than destroying
          // bytes the user may have diverged. The next reconnect re-drives the
          // delete once the file is hashable again.
          applied = false;
          log.warn('Skipping remote delete — could not hash the local file to confirm it is unmodified; keeping the local file and the DB row (a reconnect retries)', { relativePath });
        }
      }
      ...
      if (applied) {
        ...
        // Not applied: keep the DB row (the path stays watcher-suppressed for
        // this apply, and the row is left intact) and do NOT advance the cursor,
        // so the next reconnect re-delivers the delete. Record it as the lowest
        // un-applied seq this session so the catch-up wedge won't redial past it
        // and silently leapfrog this delete.
        this._noteUnappliedSeq(seq);
      }
    } finally {
      this._releasePath(relativePath);
  ...
  async _handleFileRenamed(msg) {
    ...
    let applied = false;
    // Hoisted to function scope so the delete-old + re-download-new fallback
    // below (after the try/finally) can tell _handleFileAdded that the bytes
    // now at the new path are the last-synced content just relocated — not a
    // local divergence — and skip a spurious conflict copy on a rename that
    // also changed content. Assigned inside the try once `existing` resolves.
    let movedChecksum = null;
    try {
      // Retried, so a transient Windows handle from a sync client or scanner
      // does not abort the move. `moved` records whether WE relocated the
      // source; a bare existence check on the destination is true even when an
      // unrelated local file already sits there.
      //
      // The destination directory is created only on the branch that moves
      // bytes into it, so a rename whose source is absent falls through to the
      // re-download recovery without materialising an empty tree.
      let moved = false;
      ...
      // A SYNCED row is honest only when we moved the bytes AND they match the
      // server's. Gated on `moved` rather than existence: if the source was
      // absent the move was skipped, and whatever sits at the destination is
      // not ours. Calling that SYNCED strands a row the reconcile sweep never
      // re-downloads, and later uploads those unrelated bytes to the server
      // under the renamed path.
      //
      // A pure rename carries no checksum and the moved bytes keep the one they
      // had; a checksum that differs from the last-synced value means the
      // rename also changed content. Anything not SYNCED falls through to the
      // delete-and-re-download recovery below.
      const landed = moved && nodeFs.existsSync(longPath(newFullPath));
      // movedChecksum = the LAST-SYNCED content just relocated. That is the
      // server-confirmed value (serverChecksum), NOT localChecksum: on a
      // diverged row (a pending/failed upload left localChecksum ahead of
      // serverChecksum), using localChecksum here would (a) make bytesMatch
      // wrongly true and (b) be threaded to _handleFileAdded as the
      // known-safe checksum, suppressing the conflict copy and letting the
      // re-download destroy the un-uploaded local edit. serverChecksum is
      // correct in both the synced (local==server) and diverged cases.
      movedChecksum = existing && existing.serverChecksum;
      const bytesMatch = !checksum || movedChecksum === checksum;
      // Diverged: the moved file carries an un-uploaded local edit
      // (localChecksum != serverChecksum). Never stamp such a row SYNCED — the
      // server does not have the local bytes. Keep the real diverged hashes and
      // leave it PENDING_UPLOAD so the reconcile sweep re-uploads the moved edit
      // under the new name (the pure-rename-of-a-diverged-file case).
      const diverged = !!(existing && existing.localChecksum && existing.serverChecksum &&
                          existing.localChecksum !== existing.serverChecksum);
      if (landed && bytesMatch) {
        ...
        stateDb.renameFile(oldKey, {
          ...
          localChecksum: diverged ? existing.localChecksum : (checksum || (existing && existing.localChecksum)),
          serverChecksum: diverged ? existing.serverChecksum : (checksum || (existing && existing.serverChecksum)),
          localMtime: Date.now(),
          size: size || (existing && existing.size),
          serverSeq: seq,
          status: diverged ? FILE_STATUS.PENDING_UPLOAD : FILE_STATUS.SYNCED,
        });

        // Both the filesystem move and the DB swap landed.
        applied = true;
        log.info('Renamed local file', { from: oldRelativePath, to: relativePath, pendingUpload: diverged });
      }
    } catch (err) {
    ...
    // Thread movedChecksum so _handleFileAdded's conflict-copy step recognizes
    // the relocated last-synced bytes at the new path and downloads over them
    // instead of stashing a spurious .conflict copy of content that never
    // diverged locally. When the move didn't happen (source absent) the bytes
    // on disk won't equal movedChecksum, so a genuine name-collision still
    // conflict-copies.
    await this._handleFileAdded({ fileId, relativePath, checksum, size, seq }, movedChecksum);
  }
  ...
  /**
   * Re-drives transfers the persisted state says are unfinished. Independent
   * of the seq cursor, so one that failed below it still recovers, and chained
   * onto the apply chain so it cannot race a live event for the same path.
   *
   * Pending uploads are included because a local-rename fallback whose old-path
   * delete failed leaves exactly that marker, and no further watcher event
   * fires for it — the file would otherwise sit un-uploaded indefinitely.
   */
  async _recoverPending() {
    ...
    await this._recoverPendingDeletes();

    let recovered = 0;
    // Download-recovery rows this pass converged to SYNCED (either the on-disk
    // bytes already matched, or the re-download succeeded). Used to arm the
    // catch-up wedge ONLY when the sweep did genuine download-recovery work —
    // a fail-closed non-download event (e.g. a file_removed for a locked or
    // permanently-unhashable local file) leaves no such row, so it can no longer
    // drive an endless redial loop.
    let landed = 0;
    for (const row of rows) {
      if (this._stopped || this._resyncing) break;
      const relativePath = row.relativePath;
      // A live operation owns this path, so skip it this pass as the sibling
      // recovery loops do. The periodic timer drives this off the apply chain,
      // so without the guard a reconcile could conflict-copy a freshly edited
      // file aside and re-download stale server bytes over a path an upload is
      // mid-stream on. The next tick re-drives the row.
      ...
      // A permanently-failed download (over the effective per-file cap — it
      // can't shrink) is not re-fetched every tick; cleared on a later
      // successful transfer or a resync (e.g. a raised maxFileSize).
      if (this._permanentFailures.has(relativePath)) continue;
      ...
      try {
        if (nodeFs.existsSync(longPath(fullPath))) {
          ...
          if (local === checksum) {
            ...
            landed++; // converged (bytes already on disk) — counts toward wedge recovery
            continue;
          }
        }
      ...
      this._suppressPath(relativePath);
      try {
        // As the live download paths do — without it a reconcile silently
        // clobbers a local edit that outran a failed upload.
        //
        // Deliberately no known-safe checksum here. An ERROR row conflates a
        // failed download, whose stored checksum is the undiverged marker, with
        // a failed upload, whose stored checksum is the newer diverged content.
        // Passing it would let the failed-upload case skip the copy and destroy
        // the user's newer edit. The conservative default costs at most a
        // redundant conflict file, which beats losing the edit.
        await this._maybeSaveConflictCopy(relativePath, fullPath, checksum);
        const dlOk = await this._downloadFile(relativePath, fileId, checksum, row.size);
        recovered++;
        if (dlOk) landed++; // a genuine re-download convergence
      } finally {
        this._releasePath(relativePath);
      }
    }
    ...
    // Nudge a cursor left below the catch-up tip by a failed top-of-batch
    // DOWNLOAD. Passing `landed` arms the redial ONLY when this sweep actually
    // converged download-recovery rows — never for a fail-closed non-download
    // event that left the cursor short with nothing for the sweep to land.
    this._maybeRecoverCatchUpWedge(landed);
  }

  // A failed transfer at the top of a catch-up batch wedges the session: the
  // cursor only advances on success, so it stays below the heartbeat's tip, the
  // caught-up latch never sets, and the engine reports CATCHING_UP forever
  // while the reconcile sweep quietly lands the bytes.
  //
  // The fix is a redial with the cursor UNCHANGED, not an advance from
  // reconcile — advancing could leapfrog a lower-seq event that failed and left
  // no row, dropping it silently. On redial the server replays everything from
  // the cursor: an already-landed download short-circuits on its checksum and
  // advances it idempotently, and any stranded event is re-delivered.
  //
  // Gated tightly, so it cannot thrash a healthy or still-draining session.
  _maybeRecoverCatchUpWedge(landed) {
    if (this._stopped || this._resyncing || !this._ws) return;
    if (this._state !== SYNC_STATE.CATCHING_UP) return;
    // Only arm when THIS sweep actually converged download-recovery rows. The
    // wedge exists for a failed top-of-batch DOWNLOAD whose bytes the sweep then
    // lands without advancing lastSeq; if the sweep landed nothing, the cursor is
    // short for some other reason (a fail-closed file_removed on a locked/
    // unhashable local file leaves no recoverable row) and redialing would replay
    // the same un-appliable event forever — an endless reconnect loop. Requiring
    // landed>0 also self-bounds: once the sweep has nothing left to converge, it
    // stops arming.
    if (!(landed > 0)) return;
    if (this._caughtUpAtSeq) return;                                  // a real caught-up heartbeat already latched
    if (this._catchUpPending > 0 || this._activeDownloads > 0) return; // apply queue still draining
    // Only act once a trailing heartbeat actually told us the tip is ABOVE our
    // cursor — otherwise a fresh catch-up that simply hasn't delivered its
    // heartbeat yet would be redialed spuriously on the first reconcile tick.
    let lastSeq;
    try { lastSeq = stateDb.getLastSeq(); }
    catch { return; }
    if (!(this._catchUpTipSeq > lastSeq)) return;
    // Refuse the redial only while a fail-closed event is STILL recoverable by a
    // redial — i.e. its seq is above the cursor (_lowestUnappliedSeq > lastSeq).
    // The _updateSeq watermark keeps lastSeq one below that gap, so a redial from
    // lastSeq would just re-skip the still-failing event and re-wedge; leaving the
    // visible, self-healing CATCHING_UP stall is correct. Gate on lastSeq, NOT the
    // tip: any un-applied seq is <= tip, so a tip comparison would also refuse a
    // LEGITIMATE download-recovery redial when the gap is already below the cursor
    // (unrecoverable by any redial), needlessly wedging the engine.
    if (this._lowestUnappliedSeq > 0 && this._lowestUnappliedSeq > lastSeq) {
      log.debug('Catch-up wedge redial refused — a lower-seq event failed to apply this session; leaving the recoverable stall', { lowestUnapplied: this._lowestUnappliedSeq, tipSeq: this._catchUpTipSeq });
      return;
    }
    // Only redial once the sweep has nothing left to land — a still-pending or
    // still-erroring transfer (e.g. a genuine low-disk failure) is a legitimate
    // unsync surfaced by the ERROR row, and redialing would just re-drive the
    // same failing transfer and re-wedge.
    let pending;
    try {
      pending = stateDb.getFilesByStatus(FILE_STATUS.PENDING_DOWNLOAD).length
        + stateDb.getFilesByStatus(FILE_STATUS.ERROR).length;
    } catch { return; }
    if (pending > 0) return;

    log.warn('Catch-up wedged (a stranded transfer left the sync cursor below the catch-up tip) — redialing to advance the cursor', { lastSeq, tipSeq: this._catchUpTipSeq });
    try {
      this._ws.close();
      this._setState(SYNC_STATE.CONNECTING);
      this._ws.connect(this._config.bundleId, lastSeq);
    } catch (err) {
      log.warn('Catch-up wedge redial failed', { error: err.message });
    }
  }

  // Record the lowest catch-up seq a handler failed to apply this session — a
  // fail-closed event (locked/unhashable delete, failed case-rename) OR any
  // handler that threw (caught in _onServerMessage). Reset at every WS 'open'
  // and on resync. Read by _updateSeq (which caps the cursor one below this gap
  // so a reconnect re-delivers the failed event) and by the catch-up wedge guard.
  _noteUnappliedSeq(seq) {
    if (typeof seq === 'number' && seq > 0 &&
        (this._lowestUnappliedSeq === 0 || seq < this._lowestUnappliedSeq)) {
      this._lowestUnappliedSeq = seq;
    }
  }

  /**
   * Re-drives uploads whose bytes never reached the server.
   *
   * The usual stranding is a local rename whose old-path delete failed: it
   * leaves a marker for the new path but cannot upload then, because that
   * would duplicate the still-present old path on the server. A later sweep,
   * by which time the duplicate may be gone, sends it.
   *
   * Also covers a brand-new file whose first upload failed — an error row with
   * no server id but with bytes on disk. Those strand otherwise, since the
   * download recovery needs a server id they do not have. Scoped to rows
   * without one, so this can never overlap that path or push stale local bytes
   * over a newer server copy.
   */
  async _recoverPendingUploads() {
    if (this._stopped || this._resyncing || !this._http) return;
    // Defer when the upload pool is busy. The PENDING_UPLOAD re-drive below
    // pushes directly through the pool (it no longer routes through
    // _handleLocalModify, whose no-change guard silently dead-ended a
    // crash-stranded row whose disk bytes still matched its recorded
    // localChecksum). Because the reconcile sweep runs on a timer and is NOT
    // on the apply chain, sweeping while this session's own uploads (the
    // initial-scan fan-out, a watcher upload, or a prior sweep's retry) still
    // own their rows would double-drive the same path — the PENDING_UPLOAD
    // enumeration is what dedupes a retry-in-flight row. A busy tick defers to
    // the next idle one, which is strictly better than the previous never.
    if (this._uploadPool.inFlight() > 0 || this._uploadPool.queued() > 0) return;
    let rows;
    let strandedNew;
    try {
      rows = stateDb.getFilesByStatus(FILE_STATUS.PENDING_UPLOAD);
      strandedNew = stateDb.getFilesByStatus(FILE_STATUS.ERROR)
        .filter((r) => !r.serverFileId && !r.serverChecksum && r.localChecksum);
    } catch (err) {
      log.warn('Could not enumerate pending uploads for reconcile', { error: err.message });
      return;
    }
    if (!rows.length && !strandedNew.length) return;
    ...
    for (const row of rows) {
      ...
      // Already converged: on-disk bytes match what the server has. Flip the
      // row to SYNCED rather than just skipping — a crash that landed the
      // upload server-side but never ran the SYNCED upsert leaves a
      // PENDING_UPLOAD row that would otherwise be re-hashed every sweep
      // forever AND stay invisible to _localRescan's missed-delete detection
      // (which scans SYNCED rows only), so a later local delete of the file
      // would never propagate.
      if (row.serverChecksum) {
        try {
          const local = await hashFile(fullPath);
          if (local === row.serverChecksum) {
            stateDb.upsertFile({ ...row, relativePath, localChecksum: local, status: FILE_STATUS.SYNCED });
            continue;
          }
        } catch { /* allow:silent-catch — hash probe is best-effort; fall through to re-drive the upload */ }
      }

      // A permanently-rejected file (a server 4xx that won't change on retry)
      // is not re-streamed every reconcile tick; cleared on a later success or
      // a resync.
      if (this._permanentFailures.has(relativePath)) continue;

      // Push directly through the pool — NOT via _handleLocalModify, whose
      // no-change guard (`existing.localChecksum === localChecksum`) returns
      // without uploading when a crash-stranded row's disk bytes still match
      // its recorded localChecksum, dead-ending exactly the recovery this
      // sweep is meant to perform (including the rename-fallback marker this
      // method's docstring promises to re-drive). Mirrors the strandedNew
      // branch below. _uploadFile owns the SYNCED/ERROR/PENDING_UPLOAD state
      // transitions.
      let curChecksum;
      try { curChecksum = await hashFile(fullPath); }
      catch { continue; }
      stateDb.upsertFile({
        ...row,
        relativePath,
        localChecksum: curChecksum,
        size: stat.size,
        localMtime: stat.mtimeMs,
        status: FILE_STATUS.PENDING_UPLOAD,
      });
      await this._uploadPool.run(() => this._uploadFile(relativePath, fullPath, curChecksum));
      redriven++;
    }

    // Brand-new files whose first upload failed (ERROR, no serverFileId). These
    // cannot route through _handleLocalModify — its "recorded localChecksum ===
    // on-disk hash" guard treats an un-uploaded file whose disk still matches
    // its row as "no change" and returns without uploading. Push them directly
    // through the pool exactly as the normal upload path does. Safe: the row
    // has no serverFileId and no serverChecksum, so there is no server-side
    // content this can clobber.
    for (const row of strandedNew) {
      if (this._stopped || this._resyncing) break;
      const relativePath = row.relativePath;
      const fullPath = this._safeLocalPath(relativePath);
      if (!fullPath) continue;
      if (this._isSuppressed(relativePath)) continue; // a live op owns it
      // A permanently-rejected new file (a server 4xx that won't change on
      // retry) is not re-streamed every reconcile tick; cleared on a later
      // successful upload or a resync.
      if (this._permanentFailures.has(relativePath)) continue;
      ...
      // Reflect the retry-in-flight state (PENDING_UPLOAD), so the NEXT sweep's
      // PENDING_UPLOAD enumeration — not this ERROR branch — owns the row while
      // this upload runs, avoiding a double-drive. _uploadFile re-lands ERROR on
      // failure (keeping the operator signal) or SYNCED on success.
      stateDb.upsertFile({
        ...row,
        relativePath,
        localChecksum: curChecksum,
  ...
  /**
   * Retries local deletes whose server-side delete failed. Without this a
   * single failure leaves an error row that the download recovery then
   * resurrects — the row still carries a server id while the local file is
   * gone — silently reversing the user's delete. The retry is idempotent and
   * survives a restart. A local file that has reappeared supersedes the delete
   * and is left to the upload path.
   */
  async _recoverPendingDeletes() {
    if (this._stopped || this._resyncing || !this._http) return;
    let rows;
    try { rows = stateDb.getFilesByStatus(FILE_STATUS.PENDING_DELETE); }
    catch (err) { log.warn('Could not enumerate pending deletes for reconcile', { error: err.message }); return; }
    ...
    for (const row of rows) {
      ...
      const fullPath = this._safeLocalPath(relativePath);
      if (!fullPath) continue;
      // Local file re-appeared (a recreate): the delete intent is superseded;
      // the watcher/rescan upload path owns the row from here.
      try { nodeFs.statSync(longPath(fullPath)); continue; } catch { /* still absent — proceed */ }
      // Re-read under the current state: a concurrent server event or local
      // re-create may have re-purposed the row since enumeration.
      let cur; try { cur = stateDb.getFile(relativePath); } catch { continue; }
      if (!cur || cur.status !== FILE_STATUS.PENDING_DELETE) continue;
      await this._executeDelete(relativePath, cur);
      redriven++;
    }
    if (redriven > 0) log.info('Reconcile pass re-drove stranded deletes', { count: redriven });
  }
  ...
  // Single-flighted variant used by the periodic reconcile TIMER. Chains onto
  // the apply chain like _enqueueRecover (so it serializes against live events),
  // but drops the firing if a timer-driven reconcile is already queued/running —
  // a reconcile that takes longer than RECONCILE_INTERVAL_MS must not stack up
  // behind itself. Live handlers keep calling _enqueueRecover directly (they
  // must always enqueue); only the unattended timer needs the single-flight.
  _enqueueReconcile() {
    if (this._stopped || this._reconcileQueued) return;
    this._reconcileQueued = true;
    this._applyChain = this._applyChain
      .then(() => this._recoverPending())
      .catch((err) => log.warn('Pending-transfer reconcile failed', { error: err.message }))
      .finally(() => { this._reconcileQueued = false; });
  }

  // Re-walks the synced folder and re-detects any file whose bytes diverged
  // from its tracked row. A restarted watcher begins from a clean baseline and
  // never replays what changed while it was down, which is what this recovers.
  // Serialized on the apply chain so it cannot race a live server event.
  //
  // `reason` is the log line for this particular trigger: three callers share
  // the method, so a hard-coded message would misreport two of them.
  _enqueueLocalRescan(reason) {
    if (this._stopped) return;
    log.info(reason || 'Re-scanning synced folder for local changes');
    this._applyChain = this._applyChain
      .then(() => this._localRescan())
      .catch((err) => log.warn('Local rescan failed', { error: err.message }));
  }
  ...
  async _scanLocalForUpload(serverFiles) {
    ...
    for (var i = 0; i < toUpload.length; i++) {
      ...
      // hashFilesParallel degrades an unhashable file (oversize, unreadable)
      // to { checksum: null } to stay positionally aligned. Skip it rather
      // than upsert a localChecksum=null row and stream it — a null checksum
      // makes the post-upload divergence check always fire, re-queuing the
      // file every sweep as a permanently-stranded PENDING_UPLOAD. The
      // per-file watcher path already skips a hash failure the same way.
      if (!localChecksum) {
        log.info('Skipping an unhashable local file in the cold-start scan (oversize or unreadable)', { relativePath });
        continue;
      }
  ...
  _handleHeartbeat(msg) {
    ...
    if (this._state === SYNC_STATE.CATCHING_UP) {
      ...
      if (seq <= lastSeq && (seq !== 0 || lastSeq === 0)) {
        ...
        // The catch-up-complete heartbeat carries a tip seq we haven't reached
        // — normally because catch-up events are still queued. Record it so the
        // reconcile-sweep wedge recovery can distinguish a cursor that is truly
        // stuck below the tip (a top-of-batch transfer failed and no live event
        // will re-advance it) from a fresh catch-up still draining its queue.
        this._catchUpTipSeq = seq;
      }
      this._maybeFinishCatchUp();
    }
  ...
  _updateSeq(seq) {
    ...
    try {
      // The cursor never advances to or past the first event that failed this
      // session — a fail-closed delete on a locked file, say, which leaves no
      // recoverable row. It feeds the reconnect `since`, so advancing past the
      // gap tells the server not to re-deliver that event: a remote delete
      // resurrects on this device, and the watcher can then fan the
      // resurrection back out. Capped one below the gap instead, so a reconnect
      // re-delivers it; higher-seq events re-apply idempotently afterwards.
      let target = seq;
      // The gap event itself just applied successfully: clear the watermark so
      // the cursor can advance past it. _updateSeq is only called on a
      // successful apply, so reaching it for seq === the gap means the
      // previously fail-closed event (re-delivered after a reconnect, or its
      // lock cleared) has now landed. Without this the marker persists for the
      // whole session and every later _updateSeq is capped one below the gap —
      // a frozen cursor, a permanent CATCHING_UP state, and a refused wedge
      // redial. Clear BEFORE the cap so this same call advances to `seq`.
      if (this._lowestUnappliedSeq > 0 && seq === this._lowestUnappliedSeq) {
        this._lowestUnappliedSeq = 0;
      }
      if (this._lowestUnappliedSeq > 0 && target >= this._lowestUnappliedSeq) {
        target = this._lowestUnappliedSeq - 1;
      }
      if (target > stateDb.getLastSeq()) {
        stateDb.setLastSeq(target);
      }
      stateDb.setMeta('last_sync_time', new Date().toISOString());
      metrics.setLastSeq(stateDb.getLastSeq());
      metrics.setFileCount(stateDb.getAllFiles().length);
  ...
  async _downloadFileLocked(relativePath, fileId, expectedChecksum, expectedSize, fullPath) {
    ...
    try {
      ...
      if (wanted > MAX_SYNC_FILE_BYTES) {
        ...
        // Mark permanent so the reconcile sweep does not re-attempt (and
        // re-error-log, and re-run the fail-closed conflict-copy side effect)
        // this doomed download every 5 minutes forever. The cap can't shrink
        // within a session; a raised HERMITSTASH_MAX_FILE_BYTES takes effect
        // via resync, which clears _permanentFailures. Matches the wire-layer
        // err.permanent path.
        this._permanentFailures.add(relativePath);
        return false;
      }
      ...
      stuckTimer = setTimeout(() => {
        log.warn('Download still in flight after 10 minutes (no per-call timeout — possible hung peer or very large file)',
          { relativePath, elapsedSec: Math.floor((performance.now() - downloadStart) / C.TIME.seconds(1)) });
      }, C.TIME.minutes(10));
      stuckTimer.unref();
      ...
      ok = true;
      this._permanentFailures.delete(relativePath); // a fit download clears any prior permanent mark

      log.info('Downloaded', { relativePath });
    } catch (err) {
      log.error('Download failed', { relativePath, error: err.message });
      stateDb.updateFileStatus(relativePath, FILE_STATUS.ERROR);
      // A permanent failure (over the effective per-file cap — it can't shrink)
      // must not be re-fetched every reconcile tick. Record it so the sweep
      // skips it until a successful transfer or a resync clears it.
      if (err && err.permanent) this._permanentFailures.add(relativePath);
    } finally {
      if (stuckTimer) clearTimeout(stuckTimer);
      metrics.record.download(ok);
      metrics.record.downloadDuration((performance.now() - downloadStart) / C.TIME.seconds(1));
      this._releasePath(relativePath);
  ...
  async _handleLocalModifyLocked(relativePath, fullPath, size, mtime) {
    ...
    // A same-path recreate supersedes a buffered delete for this exact path.
    // The user deleted the file (buffered with a 1s rename-detection timer)
    // and recreated it before the timer fired; this upload replaces the
    // server-side file, so the pending delete is stale. Cancel it here —
    // otherwise the timer fires after this upload completes, _executeDelete
    // re-reads the row (now carrying the fresh serverFileId from this upload),
    // and deletes the just-recreated file from the server (and every device).
    // Rename detection below only consumes OTHER paths' buffered deletes; this
    // clears the collision on relativePath itself.
    const selfPendingDelete = this._pendingDeletes.get(relativePath);
    if (selfPendingDelete) {
      clearTimeout(selfPendingDelete.timer);
      this._pendingDeletes.delete(relativePath);
      log.debug('Cancelled a buffered delete superseded by a same-path recreate', { relativePath });
    }
  ...
  // The sync root is readable — used to distinguish a genuine per-file delete
  // (root present, file gone) from a spurious mass "delete" caused by the sync
  // folder itself becoming temporarily unreadable (an unmounted NFS/SMB/USB
  // volume, a poll walk that then returns an empty tree). In the latter case
  // every tracked file looks deleted and would be removed server-side.
  _syncRootReadable() {
    try {
      const root = this._config && this._config.syncFolder;
      if (!root) return true; // no root configured — don't second-guess
      const st = nodeFs.statSync(longPath(root));
  ...
  async _handleLocalDelete(relativePath) {
    relativePath = this._identityKey(relativePath);
    // Refuse to act on a delete when the sync root itself is unreadable — the
    // event is a mount blip, not a real removal. Dropping it here avoids
    // buffering a storm of delete timers; _executeDeleteLocked re-checks at
    // fire time in case the root vanished after buffering.
    if (!this._syncRootReadable()) {
      log.warn('Ignoring a local delete — the sync folder is currently unreadable (mount blip?); not propagating', { relativePath });
      return;
    }
    const existing = this._lookupTracked(relativePath);
  ...
  async _executeDelete(relativePath, existing, opts) {
    ...
    // is no self-deadlock.
    return this._withPathLock(relativePath, () =>
      this._executeDeleteLocked(relativePath, existing, opts));
  }

  // opts.renameFallback: this delete is the OLD path of a rename whose server
  // rename failed — the "reappeared file" reappeared-guard must not fire (on a
  // case-folding volume oldPath resolves to the rename DESTINATION, which
  // legitimately exists; skipping the server delete would leave a duplicate).
  async _executeDeleteLocked(relativePath, existing, opts) {
    ...
    // The sync root vanished between buffering and firing (an unmounted volume)
    // — refuse the server DELETE so a mount blip can't storm-delete every
    // tracked file. The file may reappear when the mount returns; leave the row
    // for the next reconcile. The rename fallback runs on a live root, so this
    // does not affect it.
    if (!this._syncRootReadable()) {
      log.warn('Skipping buffered delete — the sync folder is currently unreadable (mount blip?)', { relativePath });
      return 'skipped';
    }
    // Re-read at fire time rather than trusting the snapshot taken when the
    // delete was buffered: a server event may have re-upserted the row with a
    // different id in that window. If neither the fresh read nor the snapshot
    // carries a server id the intent is stale, and skipping beats targeting the
    // wrong one. Fold-aware, or the read misses the row on a case-folding
    // volume — which is how it was located when the delete was buffered.
    ...
    // Defense-in-depth against a delete→recreate race: if the local file is
    // back on disk, the delete intent is stale (a recreate whose upload has
    // already replaced the server-side file, or a rename that moved content
    // back). Deleting now would remove the freshly-recreated file from the
    // server. The same-path cancel in _handleLocalModifyLocked normally
    // clears this, but a recreate that completed after the timer was already
    // queued behind this lock still lands here. _safeLocalPath (host platform)
    // because the delete key is a local-origin name.
    const renameFallback = !!(opts && opts.renameFallback);
    const onDisk = renameFallback ? null : this._safeLocalPath(relativePath);
    if (onDisk) {
      try { nodeFs.statSync(longPath(onDisk)); log.debug('Skipping buffered delete — the local file re-appeared (recreate)', { relativePath }); return 'skipped'; }
      catch { /* still absent — proceed with the delete */ }
    }
    // Only propagate a delete for content this client actually held synced. A
    // row with no localChecksum never had confirmed local bytes — it is a
    // download that never landed (a conflict-copy rename followed by a fast
    // download failure leaves a PENDING_DOWNLOAD/ERROR row whose serverChecksum
    // is another device's content this client never received; the watcher's
    // flush-time ENOENT then buffers a "delete"). Issuing deleteFile here would
    // remove that other device's newest content from the server for everyone.
    // A genuinely synced file, and a locally-edited file (localChecksum set),
    // both carry a localChecksum and delete normally. Leave the row for the
    // reconcile sweep rather than deleting server content we never held.
    if (!target.localChecksum) {
      log.debug('Skipping buffered delete — the row never held confirmed local content (a never-landed download), not a real local delete', { relativePath });
      return 'skipped';
    }
    // `target.relativePath` is the fold-resolved stored key (target came from
    // getFileFolded), which can differ in casing from the canonical delete key
    // on a case-folding volume — prefer it so the row mutation hits the tracked
    // row. But a rename-fallback snapshot `existing` may carry no relativePath,
    // so fall back to the canonical key (always present) rather than binding
    // `undefined` into the UPDATE/DELETE parameter (an ERR_INVALID_ARG_TYPE that
    // would abort the fallback and strand the new path).
    const delKey = (typeof target.relativePath === 'string' && target.relativePath)
    ...
    try {
      ...
      // Mutate the row under its fold-resolved key. `target` was located
      // fold-aware (getFileFolded), so on a case-folding volume its stored
      // relativePath can differ in casing from the canonical delete key — an
      // exact-match removeFile(relativePath) would then miss the tracked row
      // and leave a stale SYNCED row pointing at a now-deleted server file.
      // In the case-matching common path target.relativePath === relativePath,
      // so this is a no-op change there. Mirrors _handleFileRemoved (L1393).
      stateDb.removeFile(delKey);
      log.info('Deleted from server', { relativePath });
      return 'deleted';
    } catch (err) {
      log.error('Failed to delete from server — will retry on the next reconcile pass', { relativePath, error: err.message });
      // PENDING_DELETE, not ERROR: the local file is intentionally gone, so
      // the download-recovery sweep (which enumerates PENDING_DOWNLOAD +
      // ERROR) must NOT pick this row up and resurrect the file. The dedicated
      // _recoverPendingDeletes sweep re-drives the server DELETE instead. Keep
      // the 'failed' tri-state return — the rename fallback keys off it to
      // avoid uploading a server-side duplicate.
      stateDb.updateFileStatus(delKey, FILE_STATUS.PENDING_DELETE);
      return 'failed';
    }
  }
  ...
  async _handleLocalRename(oldPath, newPath, pending, checksum, size, mtime) {
    ...
    try {
      ...
      // The server rename is metadata only: it relocates what the server
      // already holds and uploads nothing, so its content afterwards is still
      // the pre-rename checksum. Rename detection fires on local content
      // matching the buffered delete, which can be an edit that never uploaded
      // — and recording it as the server's would claim bytes the server never
      // received. A later remote replace then reads that false last-synced
      // value, skips the conflict copy, and overwrites the edit. So record the
      // real server content, and mark the row for upload instead of leaving it
      // falsely synced.
      const priorServerChecksum = pending.existing && pending.existing.serverChecksum;
      const localMatchesServer = !!priorServerChecksum && priorServerChecksum === checksum;
      // Delete the OLD row under its fold-resolved tracked key, not the raw
      // watcher-emitted oldPath. On a case-folding volume the tracked casing
      // can differ from the watcher key; renameFile's exact-match DELETE would
      // then remove zero rows and leak a stale SYNCED row pointing at the
      // renamed file's server id (which _localRescan would later mis-delete).
      // Matches _handleFileRemoved / _executeDeleteLocked, which mutate under
      // existing.relativePath.
      const oldKey = (pending.existing && typeof pending.existing.relativePath === 'string' && pending.existing.relativePath)
        ? pending.existing.relativePath : oldPath;
      stateDb.renameFile(oldKey, {
        relativePath: newPath,
        serverFileId: pending.fileId,
        localChecksum: checksum,
        serverChecksum: localMatchesServer ? checksum : (priorServerChecksum || null),
        localMtime: mtime,
        size: size,
        serverSeq: result.seq || 0,
        status: localMatchesServer ? FILE_STATUS.SYNCED : FILE_STATUS.PENDING_UPLOAD,
      });
    } catch (err) {
      ...
      // Delete first, upload only if it committed, so the two legs cannot
      // diverge into a server-side duplicate.
      //
      // This path already holds the new path's lock. On a case-folding volume a
      // case-only rename folds both paths onto the same key, so the ordinary
      // delete — which re-acquires that non-reentrant mutex — self-deadlocks.
      // When the keys collide the locked body runs directly under the held
      // lock, and it re-checks suppression, so nothing is lost.
      const sameLock = this._suppKey(oldPath) === this._suppKey(newPath);
      const outcome = sameLock
        ? await this._executeDeleteLocked(oldPath, pending.existing, { renameFallback: true })
        : await this._executeDelete(oldPath, pending.existing, { renameFallback: true });
  ...
  // `uploadedChecksum` is of the exact bytes the caller decided to send, hashed
  // before the stream opened, and it is what the row records on success — never
  // a post-upload re-read of the disk. The stream reads lazily, so a concurrent
  // writer can change the file mid-send; storing a re-read would mark the row
  // synced against bytes the server never got, and the next watcher event would
  // see them match and never re-upload — a lost update. Recording what was
  // actually sent, and re-detecting drift below, leaves the newer bytes pending.
  async _uploadFile(relativePath, fullPath, uploadedChecksum) {
    ...
    const stuckTimer = setTimeout(() => {
      log.warn('Upload still in flight after 10 minutes (no per-call timeout — possible hung peer or very large file)',
        { relativePath, elapsedSec: Math.floor((performance.now() - uploadStart) / C.TIME.seconds(1)) });
    }, C.TIME.minutes(10));
    stuckTimer.unref();
    let ok = false;
    try {
      // The breaker wraps the retry loop, so a fully-exhausted retry counts as
      // one failure rather than three, and an open breaker fast-fails without
      // retrying at all.
      //
      // A permanent server rejection is caught inside the wrapped call and
      // returned as a sentinel: that stops the retry AND lets the breaker count
      // a success, so a handful of un-uploadable files cannot open it and stall
      // every other file. A transient failure still throws through to both.
      ...
      if (result && result._permanentUploadError) {
        ...
        stateDb.updateFileStatus(relativePath, FILE_STATUS.ERROR);
        // Permanent — the reconcile sweep must not re-stream this doomed upload
        // every tick. Cleared by a later successful upload or a resync.
        this._permanentFailures.add(relativePath);
        ...
        this._permanentFailures.delete(relativePath); // a successful upload clears any prior permanent mark
        ...
        // The file VANISHED between the caller's hash and upload completion —
        // the user deleted it while its (first) upload was in flight. Both the
        // stat and the re-hash failed with the file absent. This is a local
        // delete, not a content edit: leaving a PENDING_UPLOAD row here strands
        // an immortal zombie (the reconcile upload sweep's statSync fails every
        // tick forever) while the server keeps a file the user removed. Delete
        // the just-uploaded copy from the server and drop the row; on a delete
        // failure leave PENDING_DELETE so the reconcile delete-sweep retries.
        const fileGone = !stat && !diskNow;
        if (fileGone && fileId) {
          try {
            await this._http.deleteFile(fileId);
            stateDb.removeFile(relativePath);
            log.info('File deleted mid-upload — removed the just-uploaded copy from the server', { relativePath });
          } catch (delErr) {
            const existing = stateDb.getFile(relativePath);
            stateDb.upsertFile({
              ...existing,
            ...
            log.warn('File deleted mid-upload but the server delete failed — will retry on reconcile', { relativePath, error: delErr.message });
          }
          ok = true;
        } else if (!stat || !diskNow || diskNow !== uploaded) {
          // The local file changed between the caller's hash and upload
          // completion. Keep it PENDING_UPLOAD so the watcher/reconcile
          ...
          this._enqueueLocalRescan('Re-scanning after a local file changed during upload');
      ...
      // Cleanup in a finally (mirroring _downloadFileLocked) so a throw from
      // the catch body's updateFileStatus (transient SQLITE_BUSY / closed DB
      // during teardown) can't leak the armed stuck-timer or skip the metric
      // + gauge cleanup.
      clearTimeout(stuckTimer);
      metrics.record.upload(ok);
      metrics.record.uploadDuration((performance.now() - uploadStart) / C.TIME.seconds(1));
      this._activeOps = Math.max(0, this._activeOps - 1);
      metrics.setActiveOps(this._activeOps);
   ...
   * b.safePath, which adds NUL / C0-control / bidi-override / encoded-separator
   * / absolute-rel / Windows-reserved-name / trailing-dot or -space /
   * NTFS-ADS-marker refusals on top of the traversal check the hand-rolled
   * lexical guard did alone.
   *
   * The returned path is used for real filesystem reads and writes, so it is
   * resolved with the RUNTIME platform's path semantics — a foreign-platform
   * override yields a path in the foreign separator (e.g. "\data\x" on POSIX)
   * that does not address the host filesystem. The Windows-naming strictness for
   * server-origin names — so a POSIX peer cannot push a name a Windows peer later
   * can't materialize — is enforced separately via a windows-platform validate()
   * on the relative name, which refuses reserved names, trailing dot/space, the
   * NTFS ADS colon, and a backslash traversal regardless of the host OS. On a
   * Windows host the runtime resolve already applies those rules, so the extra
   * pass is skipped.
   */
  _safePath(relativePath) {
    // realpath:true runs b.safePath's symlink-escape walk (resolves the deepest
    // existing ancestor and refuses a path that escapes the sync root through a
    // symlinked directory). Anchoring it on this single resolve — the gate EVERY
    // write path uses, including the reconcile/recover sweep — closes the window
    // where the live-event intake guard validated the row but a symlink planted
    // before the later reconcile tick redirected the write. It is a no-op for a
    // not-yet-existing path (a normal new-file download), so it doesn't refuse a
    // legitimate fresh write.
    var full = b.safePath.resolveOrNull(this._config.syncFolder, relativePath, { realpath: true });
    if (!full) return null;
    if (process.platform !== 'win32' &&
        !b.safePath.validate(this._config.syncFolder, relativePath, { platform: 'windows' }).ok) {
      return null;
    }
    return full;
  }
