...
class SyncEngine extends EventEmitter {
  ...
  _handleCaRotation(msg) {
    ...
    try {
      ...
      _renameWithRetry(longPath(certNext), longPath(mtls.cert));
      _renameWithRetry(longPath(keyNext),  longPath(mtls.key));
      _renameWithRetry(longPath(caNext),   longPath(mtls.ca));
    } catch (err) {
  ...
  async _reconcileCaseRename(existing, relativePath, fullPath, fileId, checksum, size, seq) {
    ...
    try {
      if (oldFullPath && oldFullPath !== fullPath && nodeFs.existsSync(longPath(oldFullPath))) {
        ...
        _renameWithRetry(longPath(oldFullPath), longPath(tmp));
        _renameWithRetry(longPath(tmp), longPath(fullPath));
      }
    } catch (err) {
    ...
    const bytesMatch = (existing.localChecksum || existing.serverChecksum) === checksum;
  ...
  async _handleFileRenamed(msg) {
    ...
    try {
      ...
      // replay to re-apply cleanly). Fold-aware lookup so the old key
      // resolves on a case-folding host.
