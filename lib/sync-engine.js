...
const { hashFile } = require('./checksum');
...
class SyncEngine extends EventEmitter {
  ...
  getStatus() {
    return {
      ...
      pendingUploads: this._uploadQueue.length,
    };
  }
  ...
  async _handleFileRemoved(msg) {
    ...
    const fullPath = path.join(this._config.syncFolder, relativePath);
    try {
  ...
  async _initialSync() {
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (existing && existing.localChecksum === file.checksum) {
  ...
  async _scanLocalForUpload(serverFiles) {
    ...
    for (const fullPath of localFiles) {
      ...
      if (this._watcher && this._watcher._isIgnored(relativePath)) continue;

      if (!serverPaths.has(relativePath)) {
  ...
  async _downloadFile(relativePath, fileId, expectedChecksum) {
    ...
    try {
      ...
      await this._http.downloadFile(fileId, fullPath);
    ...
    // Restore state if we were synced before
    if (prevState === SYNC_STATE.SYNCED || prevState === SYNC_STATE.CATCHING_UP) {
