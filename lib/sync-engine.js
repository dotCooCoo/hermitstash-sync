...
class SyncEngine extends EventEmitter {
  constructor(config, apiKey) {
    ...
    // Per-target circuit breaker on the upload path. The b.retry.withRetry
    // call inside _uploadFile handles transient blips; the breaker handles
    // the sustained-failure case where every retry will fail anyway (server
    // down, network partition, mTLS cert expired). After UPLOAD_CB_FAILURES
    // consecutive failures the breaker opens for UPLOAD_CB_COOLDOWN_MS and
    // fast-fails new uploads, then probes — keeping the daemon from
    // spinning the retry loop against a flapping server.
    //
    // Constructed via `new b.circuitBreaker.CircuitBreaker(name, opts)`
    // rather than `b.circuitBreaker.create(opts)` because the framework's
    // current `create()` factory passes the opts object into the
    // positional `name` slot of the class constructor (upstream bug in
    // blamejs v0.9.7); going through the class directly avoids it.
    this._uploadBreaker = new b.circuitBreaker.CircuitBreaker('upload', {
  ...
  async _checkCertExpiry() {
    ...
    try {
      ...
      var daysLeft = Math.floor((endDate.getTime() - Date.now()) / C.TIME.days(1));
      ...
      fs.writeFileSync(this._config.mtls.cert, renewed.clientCert, { mode: 0o644 });
      fs.writeFileSync(this._config.mtls.key,  renewed.clientKey,  { mode: 0o600 });
      if (this._config.mtls.ca && renewed.caCert) {
        fs.writeFileSync(this._config.mtls.ca, renewed.caCert, { mode: 0o644 });
      }
  ...
  async _uploadFile(relativePath, fullPath) {
    ...
    let ok = false;
    try {
      // _uploadBreaker.wrap(): fast-fails with code `CIRCUIT_OPEN` when
      // the breaker is open so the retry loop below doesn't fire at all.
      // One retry-loop invocation = one breaker call (the breaker counts
      // a fully-exhausted retry as a single failure, not three).
      //
      // b.retry.withRetry inside: full-jitter exponential backoff,
      // classifier refuses to retry obvious bugs (TypeError, ReferenceError)
      // and only retries network-shaped errors. Crypto-strength jitter
      // so retry timing isn't predictable.
      const result = await this._uploadBreaker.wrap(() => b.retry.withRetry(
        async (attempt) => {
          if (attempt > 1) {
        ...
        },
        {
          ...
          jitterFactor: 0.5,
          // Anything that throws is upload-pipeline trouble — let
          // b.retry retry it (default classifier is too strict for
          // application-shaped HTTP errors that don't carry node net codes).
          isRetryable:  () => true,
        },
      ));
