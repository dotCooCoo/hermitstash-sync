...
const conflictPath = require('./conflict-path');
...
class SyncEngine extends EventEmitter {
  constructor(config, apiKey) {
    ...
    this._uploadPool = taskPool.create({ concurrency: cc });
    metrics.registerPoolStatsProvider(() => ({
      inflight:   this._uploadPool.inFlight(),
      queueDepth: this._uploadPool.queueDepth(),
    }));
  }
  ...
  async _maybeSaveConflictCopy(relativePath, fullPath, incomingChecksum) {
    try {
      ...
      const conflict = conflictPath.build(fullPath);
      nodeFs.renameSync(fullPath, conflict);
  ...
  async _handleFileRenamed(msg) {
    ...
    try {
      ...
      if (!nodeFs.existsSync(newDir)) nodeFs.mkdirSync(newDir, { recursive: true });

      // Move the file locally
      if (oldFullPath && nodeFs.existsSync(oldFullPath)) {
        nodeFs.renameSync(oldFullPath, newFullPath);
      }
  ...
  async _initialSync() {
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (fullPath && nodeFs.existsSync(fullPath)) {
  ...
  _walkDir(dir) {
    ...
    for (const entry of entries) {
      ...
      try {
        const lstat = nodeFs.lstatSync(fullPath);
  ...
  _cleanupTempFiles() {
    try {
      const files = nodeFs.readdirSync(this._config.syncFolder);
      for (const f of files) {
        if (f.includes('.tmp.')) {
          try { nodeFs.unlinkSync(nodePath.join(this._config.syncFolder, f)); } catch {}
        }
      }
    } catch {}
  }
