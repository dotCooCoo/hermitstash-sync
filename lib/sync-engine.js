...
class SyncEngine extends EventEmitter {
  ...
  async _maybeSaveConflictCopy(relativePath, fullPath, incomingChecksum) {
    try {
      ...
      // incoming bytes. A null (indeterminate) hash falls through to the rename.
  ...
  async _handleFileRemoved(msg) {
    ...
    try {
      ...
      if (nodeFs.existsSync(longPath(fullPath))) {
        ...
        } else if (lastSynced && !actualLocal) {
          // Indeterminate: the file is tracked but we could NOT hash it (it grew
          // past the size ceiling, became a special file, or is unreadable), so
          // we cannot confirm it still matches what we last synced. Fail CLOSED —
          // keep the local file and the DB row and do NOT advance seq, rather
          // than destroying bytes the user may have diverged. The next reconnect
          // re-drives the delete once the file is hashable again. Mirrors the
          // conflict-preserve failure branch above.
          applied = false;
