...
const bRetry = require('../vendor/blamejs/lib/retry');
...
class SyncEngine extends EventEmitter {
  ...
  async _uploadFile(relativePath, fullPath) {
    ...
    try {
      // b.retry.withRetry: full-jitter exponential backoff, classifier
      // refuses to retry obvious bugs (TypeError, ReferenceError, etc.)
      // and only retries network-shaped errors. Crypto-strength jitter
      // (vs Math.random) so retry timing isn't predictable.
      const result = await bRetry.withRetry(
        async (attempt) => {
          if (attempt > 1) {
