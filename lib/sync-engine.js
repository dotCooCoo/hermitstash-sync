...
class SyncEngine extends EventEmitter {
  constructor(config, apiKey) {
    ...
    // Coerce via Math.trunc(Number(...)), NOT bitwise `| 0`: ToInt32 wraps a
    // config value >= 2^31 to a negative number, which Math.min/Math.max would
    // clamp to 1 (a silent throttle to single-file uploads) instead of the
    // intended 16. NaN / 0 / undefined fall through to the default.
  ...
  // registerPoolStatsProvider keeps a single provider slot (last writer wins);
  // stop() clears it (registerPoolStatsProvider(null)) so the metrics module
  // drops its reference to this engine + pool rather than sampling a drained
  // pool. Called from BOTH the constructor and start() so a stop()+start()
  // reuse of the same engine instance — which the auto-update failed-install
  // recovery does (cli.js: engine.stop() then engine.start() on install
  // failure) — RE-registers the provider; otherwise pool telemetry
  // (upload_pool_inflight / upload_pool_queue) freezes at its pre-stop values
  // for the rest of the daemon's life. The upload pool itself persists across
  // the restart, so the closure over this._uploadPool stays valid.
  ...
  // Prove THIS runtime can complete a real TLS 1.3 mutual handshake with a
  // candidate identity — the capability that structural validation (parse +
  // key-match + chain) does NOT cover. node:tls (hence OpenSSL) can parse and
  // chain-verify a certificate whose signature algorithm it cannot drive
  // through the TLS record layer; presenting or verifying that algorithm's
  // CertificateVerify then fails the handshake. Runs an in-process loopback:
  // the incoming leaf/key is the identity on BOTH ends and the incoming CA is
  // the trust anchor. Because `requestCert` is on, the client both signs its
  // own CertificateVerify (exercising the new key's TLS-layer signing) and
  // verifies the peer's (exercising verification) before its secureConnect
  // fires — so the client callback alone proves both directions of the
  // algorithm. `rejectUnauthorized` is off and server-identity checking is
  // disabled so that an EKU / SAN / hostname mismatch on the reused client
  // leaf cannot masquerade as a capability gap: only a genuine TLS-layer
  // algorithm failure prevents secureConnect. Resolves `false` (never throws /
  // rejects) on any error or the timeout — fail-closed, so an unusable
  // algorithm is declined, not committed. Mirrors the server's _loopbackVerify
  // (hermitstash-private lib/mtls-migrate.js) adapted to the client's inputs
  // (the client holds the CA cert, not the CA key, so it reuses the leaf on
  // both ends rather than minting an ephemeral server leaf).
  // codeql[js/disabling-certificate-validation] intentional: throwaway
  // in-process 127.0.0.1 loopback that carries no application data and exists
  // solely to test whether this runtime can drive the algorithm through TLS;
  // the daemon's real server connections keep full verification + SPKI pinning.
  ...
  async start(ignorePatterns, includePatterns) {
    ...
    this._ws.on('open', () => {
      ...
      // was ERROR is dropped by _onLocalChange's early return (which returns on
      // DISCONNECTED/ERROR), and nothing re-detects it because the WS-reconnect
      // path has no local-folder reconcile of its own (only the watcher's own
      // 'rescan' does) — so recover it below with a _localRescan once the
      // connection is back. RECONNECTING-window changes are NOT dropped:
      // _onLocalChange proceeds and _handleLocalModify/_handleLocalDelete run
      // over the still-live HTTP client (uploads and server deletes travel over
      // HTTP, not the WS). So for RECONNECTING the rescan is belt-and-suspenders,
      // not corrective — keep it; do not "optimize it away" on the false premise
      // that those changes were lost.
  ...
  // arrival (== seq) order. A handler throw is caught here: it records the
  // event's seq as un-applied (_noteUnappliedSeq) so _updateSeq caps the cursor
  // one below the gap. WITHOUT that, a LATER event that applies successfully
  // would advance lastSeq (and the reconnect `since`) PAST the failed seq, so
  // the server would never re-deliver it and the add/replace/remove would be
  // silently lost — the older "just leave lastSeq below N" reasoning only held
  // when N was the last event before the disconnect. With the watermark set, a
  // reconnect dials `since` below the gap and the server replays the failed
  // event; the reconcile sweep still recovers a download that scrolled past.
  ...
  async _handleCaRotation(msg) {
    ...
    // 2b. CAPABILITY PREFLIGHT — before committing, prove THIS client's TLS
    // runtime can complete a mutual TLS 1.3 handshake with the incoming
    // identity. When the server upgrades the sync CA to a signature algorithm
    // newer than this client's bundled Node/OpenSSL (the whole point of
    // envelope agility — today ML-DSA-87, tomorrow whatever supersedes it), a
    // structurally-valid trio would otherwise be committed and then fail every
    // reconnect forever: a clean swap of an unusable cert is invisible to boot
    // recovery, which only repairs a TORN swap. Declining here (before backup +
    // swap) keeps the working identity untouched — a capability gap must never
    // brick a client. This is the client-side mirror of the server's
    // canVerifyMlDsaMtls() migration gate. Unlike a validation failure, this is
    // NOT a security reject and NOT ack'd: the server times out this client and
    // re-attempts the rotation once the client is upgraded to a runtime that
  ...
  // file forever).
  // `knownLocalChecksum` (optional) is the last-synced checksum of bytes the
  // caller KNOWS are already-synced content it just relocated — a server
  // rename that also changed content moves the old, undiverged bytes to the
  // new path. Those are not a local divergence, so conflict-copying them would
  // spam a spurious .conflict on every rename+edit. When the on-disk bytes
  // equal that value, skip the copy and let the download overwrite them.
  ...
  async _handleFileReplaced(msg) {
    ...
    try {
      ...
      //
      // Pass the last-synced value (existing.serverChecksum) as the known-local
      // checksum: on the COMMON multi-device edit (another device changed the
      // file; this device still holds the un-diverged previous version) the disk
      // bytes equal that last-synced value, so this correctly SKIPS the conflict
      // copy instead of spawning a spurious .conflict of already-synced content
      // that the watcher would then re-upload and fan out to every device. A
      // genuine local divergence (disk != last-synced AND != incoming) is still
      // preserved. Untracked path (existing===null) yields null -> safe legacy
      // behavior (conflict-copy anything that differs from incoming).
  ...
  async _recoverPending() {
    ...
    for (const row of rows) {
      ...
      // A live op owns this path — a catch-up download/replace holding the
      // per-path lock, or a watcher-driven upload streaming new bytes. Skip it
      // this pass, exactly as the four sibling recovery/rescan loops do
      // (_recoverPendingUploads, the stranded-new loop, _recoverPendingDeletes,
      // and both _localRescan loops). This loop was the only one missing the
      // guard: the periodic timer drives _recoverPending OFF the apply chain, so
      // without this a reconcile could conflict-copy the freshly-edited file
      // aside (via _maybeSaveConflictCopy, which runs OUTSIDE the per-path lock)
      // and re-download stale server bytes over a path a live upload is
      // mid-stream on — transient DB/server desync plus a bogus .conflict file.
      // The next tick re-drives the row once the live op releases the path.
      ...
      this._suppressPath(relativePath);
      try {
        // Preserve locally-diverged bytes before the re-download overwrites
        // them, mirroring the live-event download paths (_handleFileAdded,
        // _handleFileReplaced, _initialSync). Without this, a reconcile
        // re-download silently clobbers a local edit that outran a failed
        // upload (the row's on-disk bytes are ahead of serverChecksum) with no
        // conflict copy — a data-loss path the live handlers already guard.
        // _maybeSaveConflictCopy is a no-op when the local file is absent or
        // already equals the incoming checksum, so a normal never-existed-
        // locally download is unaffected. Deliberately NO known-safe checksum
        // here: an ERROR row conflates a failed download (whose localChecksum
        // is the un-diverged last-synced marker) with a failed upload (whose
        // localChecksum is the NEWER diverged content). Passing localChecksum
        // would let the failed-upload case skip the copy and the re-download
        // then destroy the user's newer edit. The conservative default —
        // conflict-copy anything that differs from the incoming bytes —
        // preserves the edit; the cost is at most a redundant .conflict file
        // for the un-diverged relocated case (1775), which beats data loss.
  ...
  // Recover a catch-up that wedged because a transfer at the TOP of the
  // catch-up batch failed. The live handler only advances lastSeq on success,
  // so a failed top-of-batch download leaves lastSeq below the trailing
  // catch-up heartbeat's tip seq; _handleHeartbeat never latches _caughtUpAtSeq
  // (the tip stays > lastSeq) and _maybeFinishCatchUp never flips to SYNCED.
  // The reconcile sweep lands the bytes but never advances lastSeq, so the
  // engine reports CATCHING_UP forever and last_sync_time goes stale even
  // though the data is synced.
  //
  // Rather than blindly advancing the cursor from reconcile — which could
  // leapfrog a lower-seq non-download event that failed to apply and left no
  // row, silently dropping it on the next since-aware reconnect — force a
  // redial with the UNCHANGED cursor. The server replays every event from
  // lastSeq: an already-landed download hits the localChecksum short-circuit
  // (advancing the cursor idempotently) and any stranded lower-seq event is
  // re-delivered and re-applied, so no event is skipped. The design's own
  // cursor recovery is reconnect-redelivery-driven; this just triggers it.
  //
  // Gated tightly so it can never thrash a healthy or still-draining session.
   ...
   *
   * Also re-drives brand-new files whose FIRST upload failed — an ERROR row
   * with no serverFileId (never reached the server) and a localChecksum (its
   * bytes are on disk). These self-strand otherwise: the download recovery in
   * _recoverPending needs a serverFileId they don't have, and enumerating only
   * PENDING_UPLOAD misses the ERROR status. Scoped to !serverFileId so this can
   * never overlap the download-recovery path or push stale local bytes over a
   * newer server copy (a row WITH a serverFileId whose failed op might have
   * been a download, not an upload).
   */
  ...
  /**
   * Re-drive PENDING_DELETE rows — local deletes whose server DELETE failed
   * transiently. Without this, a single failed DELETE would leave the row
   * ERROR and the download-recovery sweep would resurrect the file (the row
   * still carries serverFileId + serverChecksum while the local file is gone),
   * silently reversing the user's delete. The DELETE is idempotent and
   * _executeDelete re-marks PENDING_DELETE on a repeated failure, so the retry
   * persists across daemon restarts. If the local file has re-appeared, the
   * delete intent is superseded (a recreate) — leave the row to the upload
   * path instead of deleting the freshly-recreated content.
   */
  ...
  // `reason` is the operator-facing log line for THIS trigger — the method is
  // shared by three callers (watcher recovery, WS control-channel reconnect,
  // and a post-upload disk-divergence re-drive), so a hard-coded
  // "watcher recovered" message would misreport two of them. Logged after the
  // _stopped guard so it can't fire on a stopped engine.
  ...
  _updateSeq(seq) {
    ...
    try {
      // Contiguous-cursor watermark: never acknowledge (advance lastSeq) at or
      // PAST the first event that failed to apply this session — a fail-closed
      // delete on a locked/unhashable file, which advances no cursor and leaves
      // no recoverable row. lastSeq feeds the reconnect `since`, so advancing a
      // later successful event's seq past the gap would tell the server not to
      // re-deliver the failed event — silently dropping it (a remote delete then
      // resurrects on this device and the watcher can re-upload it, fanning the
      // resurrection out). Cap the advance at one below the gap so a reconnect
      // re-delivers it; higher-seq events re-apply idempotently once the gap
      // clears. No-op in normal operation (_lowestUnappliedSeq === 0). The
      // `> getLastSeq()` guard below still forbids lowering the cursor, so an
      // already-passed gap can't regress it.
      let target = seq;
  ...
  async _handleLocalRename(oldPath, newPath, pending, checksum, size, mtime) {
    ...
    try {
      ...
      // The server rename is METADATA-ONLY: it relocates the file the server
      // already holds, it does NOT upload local bytes. So the server's content
      // after the rename is still the pre-rename serverChecksum, now at newPath
      // — NOT necessarily the local `checksum`. Rename detection fires on the
      // local content matching the buffered delete's content, which can be an
      // un-uploaded edit (a failed/pending upload left localChecksum diverged
      // from serverChecksum). Recording serverChecksum = checksum there would
      // claim the server has bytes it never received; a later remote replace
      // then reads that false last-synced value, skips the conflict copy
      // (disk == claimed-server), and silently overwrites the un-uploaded
      // local edit. Record the REAL server content and, when the local bytes
      // never reached the server, mark PENDING_UPLOAD so the reconcile sweep
      // uploads them instead of leaving the row falsely SYNCED.
      ...
      //
      // This rename path already holds newPath's per-path lock (acquired by
      // _handleLocalModify before it dispatched into rename detection). On a
      // case-folding filesystem a case-only rename (e.g. 'd/a.txt' -> 'd/A.txt')
      // folds oldPath and newPath onto the SAME _suppKey, so going through
      // _executeDelete — which re-acquires that key's (non-reentrant) mutex —
      // would self-deadlock. When the keys collide, run the locked delete body
      // directly under the held lock; otherwise oldPath has a distinct key and
      // the lock-acquiring path is correct. _executeDeleteLocked re-checks
      // suppression under the lock, so the pre-lock check is not lost.
