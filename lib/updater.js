...
function createUpdater(opts = {}) {
  ...
  // Default to b.processSpawn.spawn — strips connection-string secrets
  // (DATABASE_URL, AWS_*, *_TOKEN, etc.) from the relaunched binary's
  // env. Same reason as lib/daemon.js: a stale shell-exported credential
  // shouldn't ride along into the auto-updated daemon. Tests can still
  // inject their own spawnFn via opts.
  ...
  // Outbound host allowlist for the auto-update path. Composes on
  // b.ssrfGuard, but tightens further: even valid public IPs are refused
  // unless the destination is an explicitly-named GitHub release host. A
  // compromised release index that tried to redirect downloads to e.g.
  // `evil.example.com` is rejected here before the request fires.
  // Production: api.github.com (releases JSON), github.com (redirect
  // source), objects.githubusercontent.com (CDN where binaries live).
  // Tests inject 127.0.0.1 via the mock HTTPS server agent and bypass
  // the gate via the isTestEnv branch.
  const egress = isTestEnv ? null : b.network.allowlist.create({
    allow: ['api.github.com', 'github.com', 'objects.githubusercontent.com'],
  });
  ...
  async function httpsGet(url, getOpts = {}, redirects = 0) {
    ...
    // Egress allowlist gate (production only — tests inject 127.0.0.1).
    // Refuses any host not on the GitHub-release set, including hosts a
    // hostile redirect might point at. Runs BEFORE b.httpClient.request
    // so the request never opens a socket to a disallowed host.
    if (egress) {
      try { await egress.assert(url); }
      catch (e) { throw new Error('Egress denied: ' + (e.message || String(e))); }
    }
  ...
  function start(onUpdateReady) {
    ...
    // Initial delay before the first poll (jittered window so a fleet of
    // daemons doesn't stampede GitHub's rate limit on a synchronized boot).
    const kickoff = setTimeout(runCheck, initialDelayMs);
    kickoff.unref && kickoff.unref();

    // Recurring poll via b.safeAsync.repeating — same setInterval shape but
    // with built-in onError swallowing (Promise rejections from runCheck
    // never crash the daemon), unref-by-default (the timer doesn't pin the
    // event loop on shutdown), and a clean stop() handle. Replaces the raw
    // setInterval + manual unref + manual reject swallowing.
