...
function createUpdater(opts = {}) {
  ...
  function platformAssetPattern() {
    const ext = process.platform === 'win32' ? '\\.exe$' : '$';
    // `(?:[-+][0-9A-Za-z.-]+)?` is the optional SemVer prerelease/build tail
    // (`-beta.1`, `-rc.2`, `+build.7`). Stable assets carry no suffix, so the
    // group is optional and they match byte-for-byte as before; beta-channel
    // assets named `...-v0.9.0-beta.1-<platform>-<arch><ext>` now match too.
    // The end-anchored `-<platformTag>-<arch><ext>$` is preserved, so wrong-arch
    // assets still fail and the `.mldsa.sig` sidecar still does not match.
    ...
    return new RegExp(
      'hermitstash-sync-v\\d+\\.\\d+\\.\\d+(?:[-+][0-9A-Za-z.-]+)?-' +
  ...
  function platformSignaturePattern() {
    ...
    // Same optional SemVer prerelease/build tail as platformAssetPattern so a
  ...
  // objects.githubusercontent.com URL (X-Amz-Expires, minutes). If the
  ...
  return {
    ...
    checkOnce,
    isSeaBinary,
    _internals: { performInstall, downloadTo, compareVersions, assetName, prevPathFor, readMarker, writeMarker, deleteMarker, platformAssetPattern, platformSignaturePattern, resolveFinalUrl },
  };
}
