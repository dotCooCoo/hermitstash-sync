...
function createUpdater(opts = {}) {
  ...
  async function downloadTo(url, destPath) {
    ...
    await fsp.writeFile(destPath, buf, { mode: 0o644 });
    return destPath;
  }
  ...
  function readMarker() {
    try { return b.safeJson.parse(fs.readFileSync(markerPath, 'utf8'), { maxBytes: C.BYTES.kib(64) }); } catch { return null; }
  }
  ...
  async function performInstall(version, assetPath) {
    ...
    if (process.platform !== 'win32') {
      try { fs.chmodSync(currentPath, 0o755); } catch {}
    }
  ...
  async function checkOnce(onUpdateReady) {
    ...
    const tmp = await fsp.mkdtemp(path.join(os.tmpdir(), 'hs-update-'));
    const cleanup = async () => {
    ...
    try {
      ...
      const assetPath = path.join(tmp, pollResult.asset.name);
      const sigPath   = path.join(tmp, pollResult.signature.name);
  ...
  async function checkRollback() {
    ...
    try {
      ...
      try { fs.unlinkSync(marker.prevBinaryPath); } catch {}
      if (process.platform !== 'win32') {
        try { fs.chmodSync(getExecPath(), 0o755); } catch {}
      }
    } catch (err) {
