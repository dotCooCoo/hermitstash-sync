...
function createUpdater(opts = {}) {
  ...
  function platformAssetPattern() {
    ...
    // `(?:[-+][-0-9A-Za-z.+]{1,64})?` is the optional SemVer prerelease/build tail
    // (`-beta.1`, `-rc.2`, `+build.7`, or both at once). b.selfUpdate screens the
    // assetPattern through b.guardRegex.sanitize, which refuses a repetition whose
    // boundary against the next term it cannot decide — a bare `[-0-9A-Za-z.+]{0,64}`
    // run here is refused, because the class matches the `-` and the letters of the
    // `-<platformTag>` that follows it, so the split between them floats. Leading
    // with the single `[-+]` separator pins where the tail starts, and the group is
    // `?`-quantified so it repeats at most once and adds no backtracking. The
    // {1,64} cap hardens against a hostile feed asset name. `-` is first in each
    // class so it is a literal. Stable assets (no tail) match byte-for-byte; the
    // end-anchored `-<platformTag>-<arch><ext>$` still fails wrong-arch and the
    // `.mldsa.sig` sidecar.
