...
function createUpdater(opts = {}) {
  ...
  if (!httpsAgent) {
    ...
    // future vendor bump narrows the default preference. The groups mutation
    // below is load-bearing: create()'s TLS posture sets options.groups in a
    // different preference order, and both keys must be set (Node versions
    // differ on which one OpenSSL honours). minVersion TLSv1.3 is pinned by
    // create().
  ...
  async function performInstall(version, assetPath, expectedHash) {
    ...
    if (platform === 'win32') {
      ...
      await b.selfUpdate.swap({
        ...
        backupTo:     prevPath,
        expectedHash,
        // Match the swap-side integrity re-read cap to the download cap
        // rather than riding the primitive's 1 GiB default.
        maxBytes:     MAX_ASSET_BYTES,
      });
