...
//
// A poll of the releases feed surfaces a newer build; the asset and its
// signature are downloaded over the same TLS and SSRF posture as everything
// else, and the signature is verified against the public key embedded in this
// binary before anything is swapped. The swap copies the current binary aside,
// renames the new one into place, writes a probation marker, and re-execs.
//
// The marker is what makes the update reversible. Every startup reads it: a
// fresh marker means this boot is on probation, and a timer clears it once the
// daemon has survived long enough. A stale one means the previous boot never
// got that far — so the saved binary is restored and re-launched instead.
//
// A source install polls and reports, and never swaps.
...
function createUpdater(opts = {}) {
  ...
  // An asset download redirects to a signed content CDN, and the allowlist is
  // re-checked at every hop — so that host has to be allowed or self-update
  // fails closed and silently never installs. The CDN has been renamed once
  // already, so the entry is a suffix match anchored at a label boundary: it
  // covers the old host, the current one and any future subdomain, and cannot
  // match a look-alike. The signature verified before any swap is the real
  // integrity gate; this is egress defence in depth.
  const allowedHosts = isTestEnv ? undefined : ['api.github.com', 'github.com', '.githubusercontent.com'];

  function platformAssetPattern() {
    const ext = platform === 'win32' ? '\\.exe$' : '$';
    // The optional group is the SemVer prerelease and build tail. The pattern
    // is screened by a regex guard that refuses a repetition whose boundary
    // against the next term it cannot decide — and a bare character-class run
    // here is exactly that, since the class also matches the separator and
    // letters of the platform tag that follows. Leading with the single
    // separator pins where the tail starts, and the group repeats at most once,
    // so it adds no backtracking. The bound hardens against a hostile asset
    // name from the feed.
    ...
    return new RegExp(
      'hermitstash-sync-v\\d+\\.\\d+\\.\\d+(?:[-+][-0-9A-Za-z.+]{1,64})?-' +
  ...
  function platformSignaturePattern() {
    ...
    // Same optional prerelease/build tail as platformAssetPattern (see there for
    // why the separator leads the group) so a
    // `...-v0.9.0-beta.1-<platform>-<arch><ext>.sig` sidecar matches on the beta
    // channel. The `\.exe\.sig$`/`\.sig$` anchor plus the `<arch>` prefix still
    // excludes the ML-DSA sidecar `.mldsa.sig` and every other platform's sig.
    // allow:dynamic-regex — pattern built from local platform / arch / extension only (no operator input); the runtime values come from process.platform and process.arch.
    return new RegExp(
      'hermitstash-sync-v\\d+\\.\\d+\\.\\d+(?:[-+][-0-9A-Za-z.+]{1,64})?-' +
      platformTag() + '-' + process.arch + ext
  ...
  // Streams to disk rather than buffering, hashing in flight and renaming
  // atomically into place. The URL is resolved first, so the download leg never
  // meets the redirect it would refuse.
  //
  // That resolved URL is signed and short-lived. If it expires between the
  // resolve and the download the CDN answers 403 or 410, and only a fresh
  // resolve mints a new one — so those two statuses retry the pair as a unit.
  // The classifier keys on the status code, and only the download leg carries
  // one, so an authentication or not-found failure on the resolve leg never
  // spins.
