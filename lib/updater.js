...
function createUpdater(opts = {}) {
  ...
  // Durable probation health confirmation. The daemon calls this once the
  // sync engine has started — proof the freshly-installed binary booted all
  // the way through to a running engine, which is the signal probation exists
  // to confirm. It stamps `healthyAt` into the marker (atomic rewrite,
  // preserving installedAt) so that a LATER restart which straddles the
  // probation window is recognised as a healthy restart, not a crash, and is
  // never downgraded. Without this, probation success lived only in an
  // in-memory unref()'d timer, so any unrelated restart (systemd Restart=,
  // reboot, operator stop/start, container restart) during the window left the
  // marker on disk and the next boot spuriously rolled a healthy binary back
  // to its predecessor. Idempotent + best-effort: a missing/foreign/already-
  // stamped marker is a no-op, and a write failure is swallowed (the in-memory
  // probation timer remains a fallback for the no-restart case).
  function confirmHealthy() {
    try {
      const marker = readMarker();
      if (!marker || marker.newVersion !== currentVersion || marker.healthyAt) return;
      writeMarker(currentVersion, marker.prevBinaryPath, {
        installedAt: marker.installedAt,
        healthyAt: Date.now(),
      });
    } catch { /* best-effort; the probation timer + past-probation branch still guard */ }
  }
  ...
  async function checkRollback() {
    ...
    // Past the probation window. If a prior boot of this version confirmed it
    // reached a running engine (confirmHealthy stamped healthyAt), this is a
    // healthy binary that was simply restarted across the window — complete
    // probation and never downgrade. Only roll back when there is NO health
    // evidence: the new binary never reached a running engine within the
    // window, which is the genuine botched-update crash signal.
    if (marker.healthyAt) {
      log.info(`Auto-update: v${currentVersion} completed probation (healthy restart past the window)`);
      if (marker.prevBinaryPath) {
        try { nodeFs.unlinkSync(marker.prevBinaryPath); } catch {}
      }
      deleteMarker();
    ...
    log.error(`Auto-update: v${currentVersion} never reached a healthy engine within probation — rolling back`);
    try {
...
module.exports = {
  ...
  confirmHealthy: _default.confirmHealthy,
