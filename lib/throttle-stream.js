...
function createLimiter(opts) {
  ...
  function consume(n) {
    ...
    // Need more tokens than we have. Critical correctness point:
    // deduct OPTIMISTICALLY before sleeping so concurrent callers see
    // the depleted bucket and compute their wait against the post-
    // deduction state. Without this, two parallel consume(N) calls
    // both observe the same starting tokens, both compute the same
    // waitMs, both wake at the same instant, and the bucket pays out
    // 2N tokens in one window — defeating the share-the-budget
    // semantic. Tokens may go negative; that's fine — the next caller
    // just waits longer to climb back out.
