...
async function init(opts = {}) {
  ...
  _level = LEVELS[lvl];
  // Assign (don't one-way latch): a re-init must be able to turn stdout back
  // ON, not only off. false only when explicitly requested; omitted or true
  // restores the colour-coded stdout sink. The daemon child passes
  // `stdout: false`; a foreground/re-init that wants stdout passes true.
  _stdout = (opts.stdout !== false);
...
function _emit(level, msg, data) {
  ...
  // Redact the MESSAGE string itself before our own stdout sink sees it.
  // b.logStream.emit now scrubs record.message with b.redact.redactText
  // (blamejs v0.17.13+) — a word-boundary free-text redactor that masks a
  // credential interpolated mid-message (a bearer token, an AWS key, a
  // URL-userinfo password, a PAN) while leaving surrounding prose intact,
  // which the whole-value b.redact.redact detectors used to miss. That covers
  // the FILE sink. Our stdout sink below bypasses b.logStream and writes the
  // message itself, so we run the SAME redactText here so both sinks mask
  // identically. Double-redaction on the file sink (emit redacts again) is
  // idempotent. Non-string messages pass through untouched.
  const safeMsg = typeof msg === 'string' ? b.redact.redactText(msg) : msg;
