...
function _migrateNeedsBytes(d) {
  try { d.exec('ALTER TABLE files ADD COLUMN needsBytes INTEGER DEFAULT 0'); }
    ...
    // mid-sync. Confirm the column actually landed; rethrow if it did not so
    // the failure surfaces loudly at open() instead of inside the sync loop.
    const present = d.prepare('PRAGMA table_info(files)').all().some(c => c.name === 'needsBytes');
