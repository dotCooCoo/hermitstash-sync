...
function _migrateNeedsBytes(d) {
  try { d.exec('ALTER TABLE files ADD COLUMN needsBytes INTEGER DEFAULT 0'); }
    ...
    // A current-schema DB already has the column, and the ALTER throws
    // "duplicate column name" — the one error we may safely ignore. Any
    // other failure (SQLITE_BUSY/locked, I/O) on a genuinely pre-needsBytes
    // DB would leave the column absent, and every later upsertFile (which
    // names needsBytes in its INSERT + ON CONFLICT clauses) would then throw
    // mid-sync. Confirm the column actually landed via a version-independent
    // re-probe (the SQLite duplicate-column wording is not stable across
    // versions/platforms, so a string match would be fragile); rethrow if it
    // did not so the failure surfaces loudly at open() instead of inside the
    // sync loop. The probe runs in its own try/catch: if the probe itself
    // throws (locked / I/O), the ORIGINAL ALTER error is the more actionable
    // signal (it names the operation), so surface it rather than the probe's.
    let present;
    try {
      present = d.prepare('PRAGMA table_info(files)').all().some(c => c.name === 'needsBytes');
    } catch {
      throw err;
    }
...
function open(dbPath, opts) {
  opts = opts || {};
  // Idempotent open: node:sqlite has no GC finalizer that closes the OS handle,
  // so overwriting _handle without closing first orphans the prior file handle
  // + WAL/SHM lock until process exit (and on Windows the lingering handle can
  // wedge a subsequent open). Drop any existing handle before re-opening. This
  // is a no-op on the common first-open path (_handle === null) and matches the
  // file's best-effort idempotent-teardown style elsewhere.
  if (_handle) {
    try { close(); } catch { /* allow:silent-catch — best-effort: drop the prior handle before re-opening */ }
  }
  ...
  // The needsBytes ALTER is a schema write. Only the daemon's exclusive-owner
  // path ('rename-and-recreate', the default) may issue it. Read-only consumers
  // ('refuse' — status/stats/diagnose) may be racing the live daemon that holds
  // this DB open under WAL, so they must never write: a concurrent ALTER is a
  // write-write conflict that can surface SQLITE_BUSY to the daemon mid-write,
  // and for diagnose it would mutate the very artifact being collected. Every
  // refuse-mode reader uses `SELECT *` / fixed-column reads that tolerate a
  // missing needsBytes column, so skipping the migration cannot break them.
  if (recovery === 'rename-and-recreate') _migrateNeedsBytes(_handle.db);
  return _handle.db;
}
...
function getLastSeq() {
  // Fail closed on a corrupt / non-numeric stored cursor: a bare parseInt of a
  // torn or hand-mangled value yields NaN, and NaN poisons the monotonic guard
  // in the engine (`seq > getLastSeq()` is always false against NaN) so the
  // cursor strands and every reconnect re-runs a full catch-up. Clamp to a
  // finite, non-negative integer, falling back to 0.
  const n = parseInt(getMeta('last_seq'), 10);
  return Number.isFinite(n) && n >= 0 ? n : 0;
}

function setLastSeq(seq) {
  // Refuse to persist a non-finite / negative cursor so a transient NaN upstream
  // can never be written and become a self-perpetuating stuck cursor.
...
function dumpSchema() {
  try {
    ...
    for (const o of objects) {
      ...
      // diagnose runs against a possibly-corrupt/tampered DB, and a table name
      // is the only value in this module interpolated into a SQL string rather
      // than bound. Validate it against a strict identifier allowlist and skip
      // (record null) anything that does not conform — both real tables (files,
      // sync_state) match, and a non-conforming name in this daemon-owned DB is
      // itself a corruption signal, so reporting null is the correct diagnose
      // behavior. This keeps the bound-parameter invariant uniform.
      if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(o.name)) { counts[o.name] = null; continue; }
      try {
