...
function _applyEnvOverlay(cfg) {
  ...
  function _int(name, key) {
    ...
    if (Number.isFinite(n)) overlay[key] = n;
  }
  ...
  function _str(name, key) {
    if (typeof pe[name] === 'string' && pe[name].length > 0) overlay[key] = pe[name];
  }
  function _csv(name, key) {
    if (typeof pe[name] !== 'string') return;
    overlay[key] = pe[name].split(',').map(s => s.trim()).filter(Boolean);
  }
...
function validate(config) {
  ...
  function _intGte0(name) {
    ...
    if (!Number.isInteger(config[name]) || config[name] < 0) {
      errors.push(`${name} must be a non-negative integer, got ${JSON.stringify(config[name])}`);
    }
  }
  ...
  if (config.autoUpdateChannel !== undefined &&
    ...
    errors.push(`autoUpdateChannel must be 'stable' or 'beta', got ${JSON.stringify(config.autoUpdateChannel)}`);
  }
  ...
  function _stringArray(name) {
    ...
    if (!Array.isArray(config[name])) {
      errors.push(`${name} must be an array, got ${typeof config[name]}`);
      return;
    }
    for (let i = 0; i < config[name].length; i++) {
      if (typeof config[name][i] !== 'string') {
        errors.push(`${name}[${i}] must be a string, got ${typeof config[name][i]}`);
      }
    }
  }
  ...
  if (config.pinnedServerSpki !== undefined) {
    if (!Array.isArray(config.pinnedServerSpki)) {
      errors.push(`pinnedServerSpki must be an array of "sha256/<base64>" strings`);
    } else {
      for (let i = 0; i < config.pinnedServerSpki.length; i++) {
        ...
        if (typeof p !== 'string' || !/^sha256\/[A-Za-z0-9+/]{43}=$/.test(p)) {
          errors.push(`pinnedServerSpki[${i}] must match "sha256/<43-char-base64>=", got ${JSON.stringify(p)}`);
        }
      }
    }
  }
  ...
  if (config.logLevel !== undefined &&
    ...
    errors.push(`logLevel must be one of debug/info/warn/error, got ${JSON.stringify(config.logLevel)}`);
  }
