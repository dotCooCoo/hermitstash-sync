...
const DEFAULTS = {
  ...
  tunnelMode: false,    // Reaching the server inside a Tailscale/WireGuard tunnel, where the server's external TLS leaf won't carry the tunnel (MagicDNS) hostname. When true, the client SKIPS the leaf's hostname/SAN match but STILL enforces the pinned SPKI + the mTLS chain + the post-quantum group floor — the tunnel, the pin, and mTLS bind the server's identity, so the SAN match is the one redundant layer. EXPLICIT opt-in only (never inferred from the URL); REQUIRES a non-empty pinnedServerSpki (relaxing the hostname without a pin would accept any chain-valid cert). `hermitstash-sync init --tunnel` captures the server's SPKI at enrollment and turns this on.
...
function _applyEnvOverlay(cfg) {
  ...
  _bool('HERMITSTASH_TUNNEL_MODE',         'tunnelMode');
...
// the "localhost" name). Delegates to b.ssrfGuard.isExactLoopbackName (blamejs
// v0.17.13+), which strips [] brackets + trailing dots, folds IPv4-mapped IPv6,
// lowercases, and accepts ONLY the exact "localhost" name or a loopback IP
// literal (never resolving DNS) — packaging the exact triad this used to hand-
// roll. It MUST be isExactLoopbackName, NOT b.ssrfGuard.isLoopbackHost: the
// latter also matches the RFC 6761 *.localhost subdomain reservation
// (evil.localhost === true), which an attacker can point at a public IP — and
// this function is the security gate that decides whether the enrollment code +
// API key + mTLS PRIVATE KEY may travel in cleartext http://. Shared with
// lib/http-client.js's plaintext WARN path so the two security-relevant
// classifications can never drift; the refusal POLICY (why loopback may speak
// plaintext http://) lives at the validate() call site below.
function isLoopbackHost(hostname) {
  try { return b.ssrfGuard.isExactLoopbackName(hostname); } catch { return false; }
}
...
function validate(config) {
  ...
  // Tunnel mode relaxes the TLS hostname/SAN match, so it is ONLY safe with a
  // pinned server SPKI — without a pin, any cert chaining to a trusted CA would
  // be accepted with no identity binding. Fail closed at config time so a
  // misconfigured tunnel client never dials with the hostname check off and no
  // pin (the connect-time guard in http-client.js is the defense-in-depth twin).
  if (config.tunnelMode !== undefined && typeof config.tunnelMode !== 'boolean') {
    errors.push(`${_label('tunnelMode')} must be a boolean, got ${typeof config.tunnelMode}`);
  }
  if (config.tunnelMode === true &&
      (!Array.isArray(config.pinnedServerSpki) || config.pinnedServerSpki.length === 0)) {
    errors.push(`${_label('tunnelMode')} is on but ${_label('pinnedServerSpki')} is empty — tunnel mode skips the TLS hostname check and MUST pin the server's SPKI. Re-run "hermitstash-sync init --tunnel" to capture it, or add the pin manually.`);
  }
