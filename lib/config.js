...
// Every other host over http:// would ship the Bearer API key and all sync
// metadata in cleartext with the post-quantum TLS floor absent, so it is
// refused at config-load. b.ssrfGuard.isLoopback covers the IP forms (it does
// not treat the "localhost" NAME as loopback, so special-case that).
...
function validate(config) {
  ...
  if (typeof config.server === 'string' && config.server) {
    ...
    if (parsedServer && parsedServer.protocol === 'http:' &&
        !_isLoopbackHost(parsedServer.hostname) && !_allowPlaintextServer()) {
      errors.push(
