...
async function run() {
  ...
  _block("verbatim caller-wildcard LIKE (the b.db.collection $like path) runs on real Postgres", function () {
    ...
    // escaped). Prove the identifier quoting + LIKE compose on real Postgres,
    // not just the sqlite collection path. Notes are still first/second/third here.
    var pre = _from(adapter, T, declared).whereGroup(function (g) { g.like("note", "fir%"); }).all();
    soft("LIKE 'fir%' matches only 'first' on Postgres", pre.length === 1 && pre[0]._id === "o-1");
    var inf = _from(adapter, T, declared).whereGroup(function (g) { g.like("note", "%ir%"); })
      .orderBy("_id", "asc").all();
    soft("LIKE '%ir%' matches 'first' + 'third' on Postgres",
