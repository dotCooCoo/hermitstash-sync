...
// Step 1: Bundle all source files into a single file
...
if (!fs.existsSync(BUNDLE_PATH)) {
  console.error('ERROR: Bundle was not generated.');
  process.exit(1);
}
console.log(`  Bundle: ${BUNDLE_PATH} (${(fs.statSync(BUNDLE_PATH).size / 1024).toFixed(1)} KB)\n`);

// Step 2: Generate the SEA blob from the bundle
console.log('Step 2: Generating SEA blob...');
run(`node --experimental-sea-config ${q(SEA_CONFIG)}`);
...
if (os.platform() === 'darwin') {
  ...
  try {
    run(`codesign --remove-signature ${q(EXE_PATH)}`);
  } catch {
...
console.log('Step 4: Injecting SEA blob into binary...');
const sentinel = 'NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2';

try {
  run(`npx --yes postject ${q(EXE_PATH)} NODE_SEA_BLOB ${q(BLOB_PATH)} --sentinel-fuse ${sentinel}` +
