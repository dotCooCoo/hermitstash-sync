...
'use strict';
/**
 * Workflow-side helper: sign a release artifact (a SEA binary)
 * with the framework's ML-DSA-65 release-signing key, then write
 * the signature alongside as `<artifact>.mldsa.sig`.
 *
 * Invoked from `.github/workflows/release.yml`:
 *
 *   node scripts/sign-release-artifact.js build/hermitstash-sync-vX.Y.Z-linux-x64
 *
 ...
 * signatures that operators would later refuse).
 *
...
// FIPS 204 ML-DSA-65 key byte lengths — fixed by the spec, used as
// integrity floors for the decoded env secret + in-tree pubkey.
var ML_DSA_65_PUBKEY_BYTES  = 1952;                                                    // allow:raw-byte-literal — FIPS 204 ML-DSA-65 public-key byte length
var ML_DSA_65_SECRET_BYTES  = 4032;                                                    // allow:raw-byte-literal — FIPS 204 ML-DSA-65 secret-key byte length
...
function main() {
  ...
  if (!secB64 || secB64.length === 0) {
    fail('RELEASE_PQC_SIGNING_KEY env not set. Run scripts/generate-release-signing-key.js once + set the secret on the repo.');
  }

  var secretKey = new Uint8Array(Buffer.from(secB64, 'base64url'));
  if (secretKey.length !== ML_DSA_65_SECRET_BYTES) {
    fail('RELEASE_PQC_SIGNING_KEY decodes to ' + secretKey.length +
      ' bytes; expected ' + ML_DSA_65_SECRET_BYTES + ' (FIPS 204 ML-DSA-65). Key may be corrupted.');
  }
  var publicKey = readPubKey();
  if (publicKey.length !== ML_DSA_65_PUBKEY_BYTES) {
  ...
  // against an env secret that doesn't match the committed pubkey.
  // If verify fails, refuse to write — the operator-side
  // verification path would also fail, and the workflow's
  // downstream release-create step would attach a signature that
  // no one can verify.
